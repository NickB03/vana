...
class MemoryOptimizedQueue:
    ...
    def close(self):
        ...
        # Try to notify immediately if we're in an async context
        try:
            loop = asyncio.get_running_loop()
            # Don't create a task if we're not in the right context, just mark as closed
            # The next get() call will see _closed and raise CancelledError
        except RuntimeError:
...
class SessionManager:
    ...
    def increment_subscribers(self, session_id: str):
        ...
        with self._lock:
            ...
            # Update session activity directly to avoid nested lock
            if session_id in self._sessions:
                self._sessions[session_id] = time.time()
...
class EnhancedSSEBroadcaster:
    ...
    def _start_background_cleanup(self):
        ...
        if not self._running:
            ...
            try:
                ...
                logger.warning("No event loop running, cleanup task will start later")
    ...
    async def _perform_cleanup(self):
        ...
        # Force garbage collection before cleanup for memory measurement
        gc.collect()
        ...
        if self.config.enable_metrics:
            ...
            if self._metrics.process_memory_mb > self.config.memory_critical_threshold_mb:
                logger.warning(f"Critical memory usage detected: {self._metrics.process_memory_mb:.1f}MB")
    ...
    async def _update_memory_metrics(self):
        ...
        try:
            ...
            if hasattr(self, "_process") and self.config.enable_metrics:
                try:
                    # Add timeout to prevent hanging on memory info
    ...
    async def _aggressive_cleanup(self):
        ...
        with self._lock:
            # Reduce history size for all sessions by 50%
            for session_id, events in self._event_history.items():
                if len(events) > 10:  # Keep minimum 10 events
                    new_size = max(10, len(events) // 2)
                    # Create new deque with reduced size
                    new_events = deque(list(events)[-new_size:], maxlen=new_size)
                    self._event_history[session_id] = new_events

            # Close idle queues more aggressively
            for session_id, queues in list(self._subscribers.items()):
                active_queues = []
                for queue in queues:
                    # Use more aggressive staleness check (5 minutes instead of 10)
                    if not queue._closed and not queue.is_stale(300.0):
                        active_queues.append(queue)
                    else:
                        queue.close()

                if active_queues:
                    self._subscribers[session_id] = active_queues
                else:
                    del self._subscribers[session_id]
    ...
    async def add_subscriber(self, session_id: str) -> MemoryOptimizedQueue:
        ...
        # Check memory pressure before adding new subscribers
        if self.config.enable_metrics and hasattr(self, '_metrics'):
            if self._metrics.process_memory_mb > self.config.memory_critical_threshold_mb:
                logger.warning(f"Rejecting new subscriber due to memory pressure: {self._metrics.process_memory_mb:.1f}MB")
                # Return a closed queue to prevent memory growth
                queue = MemoryOptimizedQueue(maxsize=1)
                queue.close()
                return queue
...
# Thread-safe singleton pattern for SSE broadcaster
_broadcaster: EnhancedSSEBroadcaster | None = None
_broadcaster_lock = threading.RLock()  # Reentrant lock for nested calls


def get_sse_broadcaster() -> EnhancedSSEBroadcaster:
    """Get the global SSE broadcaster instance (thread-safe singleton pattern)."""
    global _broadcaster
    ...
    if _broadcaster is None:
        with _broadcaster_lock:
            ...
            if _broadcaster is None:
                ...
                _broadcaster = EnhancedSSEBroadcaster(config)
