...
class MultiAgentResearchOrchestrator:
    ...
    def __init__(self, google_api_key: str | None = None):
        ...
        self.openrouter_model: str = os.getenv(
            "OPENROUTER_MODEL", "openrouter/openrouter/sonoma-sky-alpha"
        )
        ...
        if self.use_openrouter:
            ...
            if llm_completion is None:
                raise RuntimeError(
                    "LiteLLM is required for OpenRouter but not available"
                )
        else:
            # Gemini path
            if genai is None:
    ...
    async def start_research_session(
        self, session_id: str, research_query: str
    ) -> ResearchProgress:
        """Start a new multi-agent research session"""

        # Create agents
        agents = [
            self.create_agent_status(agent_type) for agent_type in self.AGENT_TYPES
        ]
    ...
    async def _orchestrate_research(self, session_id: str, research_query: str):
        ...
        try:
            ...
            final_agent = next(
                (a for a in progress.agents if a.agent_type == "report_writer"), None
            )
            if final_agent and final_agent.results:
                progress.final_report = final_agent.results.get(
        ...
        try:
            ...
            if self.use_openrouter:
                ...
                try:
                    ...
                    model_id = self.openrouter_model.replace(
            ...
            try:
                if self.use_openrouter:
                    ...
                    if hasattr(response, "choices") and response.choices:
                        ...
                        if hasattr(msg, "content"):
                            ...
                            content = msg.get("content", "") or getattr(
                                choice0, "text", ""
                            )
                        else:
                ...
                content = f"{agent_type.title()} analysis completed with content extraction error: {e!s}"
    ...
    async def start_research_with_broadcasting(
        self, session_id: str, research_query: str
    ):
        ...
        try:
            ...
            await self._orchestrate_research_with_broadcasting(
                session_id, research_query
            )
    ...
    async def _orchestrate_research_with_broadcasting(
        self, session_id: str, research_query: str
    ):
        ...
        try:
            ...
            phases = [
                (
                    "team_leader",
                    "Analyze this research request and create a comprehensive research strategy",
                    0.15,
                ),
                (
                    "plan_generator",
                    "Create a detailed research plan based on the strategy",
                    0.30,
                ),
                (
                    "section_planner",
                    "Break down the research plan into structured sections",
                    0.45,
                ),
                (
                    "researcher",
                    "Conduct comprehensive research based on the plan",
                    0.70,
                ),
    ...
    async def _execute_agent_phase_with_broadcasting(
        self, session_id: str, agent_type: str, prompt: str
    ):
...
research_orchestrator: MultiAgentResearchOrchestrator | None = None
