...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    def _get_csp_policy_for_path(
        self, path: str, nonce: str, request: Request = None
    ) -> str:
        ...
        is_adk_request = (
            ...
            or path.startswith("/_adk")
            or path.startswith("/apps/")
            or path.startswith("/list-apps")
            or path.startswith("/debug/")
            or path.startswith("/static/")  # ADK static assets
            or path.endswith(".js")  # JavaScript files (ADK main files)
            or path.endswith(".css")  # CSS files (ADK styling)
            or "dev-ui" in path.lower()
            or "main-" in path  # ADK main-*.js files
            or (request and "google" in request.headers.get("user-agent", "").lower())
        )
