...
class TestComponent:
    ...
    def collect_metrics(self):
        ...
        return {
            ...
            "latency": self.latency,
            "request_count": self.request_count,
            "requests_per_second": self.request_count / (time.time() - self.last_request_time)
        }
...
def test_dashboard():
    ...
    for component in components:
        ...
        logger.info(f"Health data for {component.name}: {json.dumps(health_data, indent=2)}")
    
    # Test component metrics
    logger.info("Testing component metrics...")
    for component in components:
        metrics_data = dashboard.get_component_metrics(component.name)
        logger.info(f"Metrics data for {component.name}: {json.dumps(metrics_data, indent=2)}")
    ...
    active_alerts = alert_manager.get_active_alerts()
    logger.info(f"Active alerts after resolution: {json.dumps(active_alerts, indent=2)}")
    ...
    for component in components:
        component_data = dashboard.get_historical_data(component_name=component.name, limit=5)
        logger.info(f"Historical data for {component.name}: {json.dumps(component_data, indent=2)}")
