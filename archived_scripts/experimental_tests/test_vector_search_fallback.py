...
from tests.fixtures.vector_search_fixtures import (
    ...
    real_vector_search_client
)

# Import the actual implementations
from tools.vector_search.vector_search_client import VectorSearchClient, SimpleMockVectorSearchClient
...
class TestVectorSearchFallback:
    ...
    def test_fallback_on_embedding_generation_error(self):
        ...
        with patch.object(client, '_initialize', return_value=True):
            with patch.object(client, 'generate_embedding', side_effect=Exception("Simulated embedding error")):
    ...
    def test_warning_logging_on_fallback(self, caplog):
        ...
        assert any("Falling back to mock implementation" in record.message for record in caplog.records)
    ...
    def test_mock_client_behavior(self):
        ...
        assert all("id" in result and "score" in result and "content" in result for result in results)
