...
class TestConversationContextManager(unittest.TestCase):
    ...
    def test_list_conversation_contexts(self):
        ...
        contexts = self.context_manager.list_conversation_contexts(session_id="session3")
        self.assertEqual(len(contexts), 1)
        self.assertEqual(contexts[0].user_id, "user2")
        
        # List contexts by scope
        contexts = self.context_manager.list_conversation_contexts(scope=ConversationContext.SCOPE_USER)
        self.assertEqual(len(contexts), 1)
        self.assertEqual(contexts[0].user_id, "user2")
    ...
    def test_get_relevant_contexts(self):
        ...
        context1.add_message("assistant", "What specific Python topic do you need help with?")
        ...
        context3.add_message("user", "What are some good Python libraries for data science?")
        context3.add_message("assistant", "NumPy, Pandas, and Scikit-learn are popular choices.")
        ...
        cooking_contexts = [c for c in relevant_contexts if "cook" in str(c.messages) or "pasta" in str(c.messages)]
