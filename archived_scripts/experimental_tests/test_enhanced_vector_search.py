...
def test_generate_embedding():
    ...
    try:
        ...
        logger.info(f"✅ Handled None input gracefully: {len(invalid_embedding) if invalid_embedding else 'empty'}")
...
def test_search_with_embedding(embedding: List[float]):
    ...
    try:
        ...
        if 'search_vector_store' in methods:
            ...
            if results:
                logger.info(f"✅ Successfully retrieved {len(results)} results using search_vector_store")
                ...
                logger.info("No results returned (this might be expected in test environment)")
                return []
        else:
            ...
            if 'search' in methods:
                ...
                logger.info(f"✅ Successfully retrieved {len(results)} results using search")
                return results
            else:
...
def test_search_knowledge():
    ...
    if 'search_knowledge' in methods:
        ...
        try:
            ...
            # Check if results were returned
            if results:
                logger.info(f"✅ Successfully retrieved {len(results)} knowledge results")

                # Check result structure
                for i, result in enumerate(results):
                    if all(key in result for key in ["content", "score", "source", "id", "metadata"]):
                        logger.info(f"✅ Knowledge result {i+1} has correct structure")
                    else:
                        logger.error(f"❌ Knowledge result {i+1} is missing required fields")
            else:
                logger.info("No knowledge results returned (this might be expected in test environment)")

            # Test with invalid query
            try:
                invalid_results = client.search_knowledge(None)
                logger.info(f"✅ Handled None query gracefully: {len(invalid_results) if invalid_results else 'empty'}")
            except Exception as e:
...
def main():
    ...
    logger.info(f"- Search with embedding: {'✅ Success' if search_results else '❌ Failed'}")
    logger.info(f"- Knowledge search: {'✅ Success' if knowledge_results else '❌ Failed'}")
