...
def test_secure_mcp_client(mcp_credentials):
    ...
    from tools.security import Role
    ...
    try:
        ...
        if result.get("success", False):
            ...
            logger.warning(f"⚠️ Failed to store entity: {result.get('error', 'Unknown error')}")
    ...
    try:
        ...
        if "entity" in result:
            ...
            logger.warning(f"⚠️ Failed to retrieve entity: {result.get('error', 'Unknown error')}")
    except CircuitBreakerOpenError as e:
        logger.error(f"❌ Circuit breaker open: {str(e)}")
    except Exception as e:
...
def test_audit_logging():
    ...
    if verification["verified"]:
        ...
        logger.info(f"Checked {verification['files_checked']} files and {verification['entries_checked']} entries")
    else:
...
def test_circuit_breaker():
    ...
    from tools.resilience import registry, CircuitBreaker, circuit_breaker
