...
class TestCase:
    ...
    def execute_step(self, func, *args, **kwargs):
        ...
        try:
            ...
            logger.info(f"Step {self.current_step['name']} {self.current_step['status']} in {self.current_step['duration']:.2f} seconds")
    ...
    def assert_not_equal(self, expected, actual, message=None):
        ...
        """
        if expected == actual:
            error_message = message or f"Expected {expected} to be different from {actual}"
            logger.error(error_message)
            raise AssertionError(error_message)
