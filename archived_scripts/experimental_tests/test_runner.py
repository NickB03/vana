...
class TestRunner:
    ...
    def _load_config(self):
        ...
        try:
            ...
            logger.error(f"Error loading test configuration from {self.config_path}: {e}")
            return {
    ...
    def run_scenario(self, scenario):
        ...
        try:
            ...
            # Update the result
            result["status"] = "passed" if scenario_result.get("success", False) else "failed"
        ...
        logger.info(f"Scenario {scenario_name} {result['status']} in {result['duration']:.2f} seconds")
    ...
    def run_tests(self):
        ...
        logger.info(f"Passed: {self.results['passed']}, Failed: {self.results['failed']}, Skipped: {self.results['skipped']}, Total: {self.results['total']}")
    ...
    def save_results(self, output_path=None):
        ...
        if not output_path:
            ...
            output_path = os.path.join(results_dir, f"test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
