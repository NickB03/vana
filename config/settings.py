...
class Settings(BaseSettings):
    ...
    vana_model: str = Field(
        ...
        description="Default LLM model for VANA agent"
    )
    
    # Google Cloud Configuration
    google_project_id: str = Field(
        default="analystai-454200",
    ...
    memory_similarity_top_k: int = Field(
        default=5,
        description="Number of top results for similarity search"
    )
    
    memory_vector_distance_threshold: float = Field(
        default=0.7,
        description="Distance threshold for vector search"
    )
    
    memory_cache_size: int = Field(
        default=1000,
        description="Local memory cache size"
    )
