...
from sync_knowledge import get_repository_files, read_file_content, chunk_text, generate_embeddings, update_vector_search
...
def parse_arguments():
    ...
    parser.add_argument("--max-files", type=int, default=5, help="Maximum number of files to process")
    parser.add_argument("--query", default="How does the Vector Search integration work?",
...
def main():
    ...
    files = get_repository_files(args.repo_path, file_types, exclude_dirs, args.max_files)
    ...
    if success:
        ...
        if query_success:
            ...
            logger.info("This is expected if the index doesn't support direct updates or hasn't been populated yet")
            logger.info("The test is still considered successful if embeddings were generated correctly")
    else:
