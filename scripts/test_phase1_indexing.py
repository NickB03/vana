...
def should_process_file(file_path: Path) -> bool:
    ...
    exclude_dirs = [
        'archive/', 'memory-bank/', 'node_modules/', '.git/', '__pycache__/', '.pytest_cache/',
        'build/', 'dist/', '.venv/', 'venv/', '.mypy_cache/', '.coverage/', 'htmlcov/',
        '.tox/', '.env/', 'logs/', 'tmp/', 'temp/', '.DS_Store', '.idea/', '.vscode/settings.json'
    ]
    ...
    legacy_patterns = [
        'old_', 'legacy_', 'deprecated_', 'backup_', 'temp_', 'test_temp', 
        '_backup', '_old', '_legacy', '_deprecated', 'scratch_', 'experiment_'
    ]
    ...
    one_time_patterns = [
        'setup_', 'install_', 'migration_', 'fix_', 'hotfix_', 'patch_',
        'debug_', 'troubleshoot_', 'analyze_', 'benchmark_'
    ]
    ...
    exclude_extensions = [
        '.pyc', '.pyo', '.pyd', '.so', '.dll', '.dylib', '.exe', '.zip', '.tar', '.gz',
        '.jpg', '.jpeg', '.png', '.gif', '.ico', '.svg', '.pdf', '.mp4', '.mov',
        '.log', '.pid', '.lock', '.sqlite', '.db', '.cache'
    ]
    ...
    core_files = [
        ...
        'setup.py', 'main.py', '__init__.py', 'Dockerfile', 'docker-compose.yml'
    ]
...
def main():
    ...
    # Quality assessment
    quality_ratio = results['indexed_files'] / cleanup_results['total_candidates'] if cleanup_results['total_candidates'] > 0 else float('inf')
