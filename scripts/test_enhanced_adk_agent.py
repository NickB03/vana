...
class ADKAgentTester:
    ...
    def test_tool_imports(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            from vana_adk_clean.vana_agent.agent import (
                ...
                file_exists_tool, vector_search_tool, web_search_tool,
                kg_query_tool, kg_store_tool, get_health_status_tool,
                get_info_tool, help_tool, root_agent
            )
    ...
    def test_tool_functions(self) -> List[Dict[str, Any]]:
        ...
        try:
            from vana_adk_clean.vana_agent.agent import (
                echo_tool, get_info_tool, help_tool, vector_search_tool,
                web_search_tool, kg_query_tool, kg_store_tool, get_health_status_tool
            )
            ...
            results.append({
                ...
                "test": "Echo Tool",
                "status": "PASSED" if "Echo: test message" in echo_result else "FAILED",
            ...
            results.append({
                "category": "Tool Functions",
                "test": "Vector Search Tool",
                "status": "PASSED" if "Vector search results" in vector_result else "FAILED",
            ...
            results.append({
                "category": "Tool Functions",
                "test": "Web Search Tool", 
                "status": "PASSED" if "Web search results" in web_result else "FAILED",
            ...
            results.append({
                "category": "Tool Functions",
                "test": "KG Query Tool",
                "status": "PASSED" if "Knowledge graph results" in kg_result else "FAILED",
                "details": f"Contains results: {'Knowledge graph results' in kg_result}"
            })
            
            # Test knowledge graph store
            kg_store_result = kg_store_tool("test_entity", "test_type", "test observation")
            results.append({
                "category": "Tool Functions",
                "test": "KG Store Tool",
                "status": "PASSED" if "Successfully stored" in kg_store_result else "FAILED",
            ...
            results.append({
                ...
                "status": "PASSED" if "System Health Status" in health_result else "FAILED",
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if not failed_tests:
            recommendations.append("✅ All automated tests passed! Ready for manual testing.")
        else:
            recommendations.append(f"❌ {len(failed_tests)} tests failed. Review and fix issues before manual testing.")
...
def main():
    ...
    for result in report["results"]:
        ...
        print(f"{status_icon} {result['category']} - {result['test']}: {result['status']}")
