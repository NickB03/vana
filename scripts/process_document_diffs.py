...
class DiffProcessor:
    ...
    def __init__(self, chunk_size: int = 500, chunk_overlap: int = 50):
        ...
        if not self.kg_available:
            logger.warning(
                "Knowledge Graph is not available. Changes will not be reflected in Knowledge Graph."
            )

        if not self.vs_available:
            logger.warning(
                "Vector Search is not available. Changes will not be reflected in Vector Search."
            )
    ...
    def parse_diff_file(self, diff_file: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "reason": str(e), "file_path": diff_file}

    def create_chunks_from_diff(
        ...
        for section_idx, section in enumerate(sections):
            ...
            if added_content:
                ...
                added_chunks = self._chunk_text(
                    added_content, self.chunk_size, self.chunk_overlap
                )
    ...
    def update_knowledge_base(self, chunks: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        added_chunks = [
            chunk for chunk in chunks if chunk["metadata"]["change_type"] == "added"
        ]
        removed_chunks = [
            chunk for chunk in chunks if chunk["metadata"]["change_type"] == "removed"
        ]
        ...
        if self.kg_available:
            for chunk in added_chunks:
                try:
                    ...
                    logger.error(
                        f"Error processing entities for Knowledge Graph: {str(e)}"
                    )
        ...
        # Log statistics
        logger.info(
            f"Processed {stats['chunks_processed']} chunks: {len(added_chunks)} added, {len(removed_chunks)} removed"
        )
        logger.info(f"Updated {stats['vector_search_updates']} chunks in Vector Search")
        logger.info(
            f"Extracted {stats['entities_extracted']} entities and updated {stats['entities_updated']} in Knowledge Graph"
        )
...
def main():
    ...
    parser.add_argument(
        "--input-dir", help="Path to the directory containing diff files"
    )
    parser.add_argument("--output-dir", help="Output directory for processed documents")
    parser.add_argument(
        "--chunk-size", type=int, default=500, help="Maximum chunk size in characters"
    )
    parser.add_argument(
        "--chunk-overlap",
        type=int,
    ...
    if result.get("success", False):
        if args.input_file:
            ...
            logger.info(
                f"Successfully processed {result.get('successful', 0)}/{result.get('total_files', 0)} diff files"
            )
