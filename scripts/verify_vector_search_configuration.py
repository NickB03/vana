...
def parse_arguments():
    ...
    parser.add_argument(
        "--deployed-index-id", help="Deployed Index ID", default="vanasharedindex"
    )
...
def get_configuration(args) -> dict[str, Any]:
    ...
    if not config["project_id"]:
        logger.error(
            f"Project ID not provided. Set {ENV_PROJECT_ID} environment variable or use --project"
        )
        sys.exit(1)
    ...
    if not config["endpoint_id"]:
        logger.error(
            f"Endpoint ID not provided. Set {ENV_ENDPOINT_ID} environment variable or use --endpoint-id"
        )
        sys.exit(1)
    ...
    config["deployed_index_id"] = (
        ...
        or os.environ.get(ENV_DEPLOYED_INDEX_ID)
        or "vanasharedindex"
    )
    ...
    if not config["credentials_path"]:
        logger.warning("Credentials path not provided. Using default credentials.")
...
def validate_environment(config: dict[str, Any]) -> bool:
    ...
    if not os.environ.get(ENV_DEPLOYED_INDEX_ID) and not config.get(
        "deployed_index_id"
    ):
    ...
    if missing_vars:
        ...
        logger.warning(
            "Some variables were provided as command line arguments, but setting environment variables is recommended for production use."
        )

    # Check credentials
    if not os.environ.get(ENV_CREDENTIALS) and not config.get("credentials_path"):
        logger.warning(
            f"Missing {ENV_CREDENTIALS} environment variable. Using default application credentials."
        )
        logger.warning(
            "This may work for development but is not recommended for production."
        )
    elif config.get("credentials_path") and not os.path.exists(
    ...
    try:
        ...
        if config.get("credentials_path"):
            try:
                ...
                logger.info(
                    f"Successfully loaded credentials from {config['credentials_path']}"
                )
        ...
        logger.info(
            f"Successfully initialized Vertex AI with project {config['project_id']} in {config['location']}"
        )
...
def validate_service_account_permissions(
    config: dict[str, Any], credentials: Any
) -> bool:
    ...
    try:
        ...
        logger.info(
            f"2. Navigate to IAM & Admin > IAM for project {config['project_id']}"
        )
...
def validate_vector_search_resources(config: dict[str, Any], credentials: Any) -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Failed to find index endpoint {config['endpoint_id']}: {str(e)}"
            )
            logger.error(
                "Check that the endpoint ID is correct and exists in the specified project and location."
            )
            return False
        ...
        if not deployed_index_found:
            logger.error(
                f"Deployed index {config['deployed_index_id']} not found on endpoint {config['endpoint_id']}"
            )
...
def validate_embedding_generation(config: dict[str, Any], credentials: Any) -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Successfully generated embedding with {len(embedding_values)} dimensions"
            )
        ...
        logger.error(
            "Failed to import embedding libraries. Make sure vertexai is installed."
        )
...
def test_vector_search_query(config: dict[str, Any], credentials: Any) -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Successfully retrieved {result_count} results from Vector Search"
            )

            if result_count == 0:
                logger.warning(
                    "No results found. This might indicate that the index is empty."
                )
                logger.warning(
                    "This is not necessarily an error, but you may want to populate the index with data."
                )
...
def generate_validation_report(
    config: dict[str, Any], validations: dict[str, bool]
) -> str:
    ...
    if all_passed:
        report_lines.append(
            "Overall Status: ✅ All validations passed! Vector Search is properly configured."
        )
    else:
        report_lines.append(
            "Overall Status: ❌ Some validations failed. See details above."
        )
