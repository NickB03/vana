    ...
    try:
        ...
        export_data = {
            ...
            "source": {
                ...
                "namespace": mcp_client.namespace,
            },
            "entities": entities,
...
def import_memory_data(input_file: str, conflict_strategy: str = "skip") -> bool:
    ...
    try:
        ...
        logger.info(
            f"Found {len(entities)} entities and {len(relationships)} relationships in import file"
        )
        ...
        for entity in entities:
            try:
                ...
                if not entity_name or not entity_type:
                    logger.warning(
                        f"Skipping entity with missing name or type: {entity}"
                    )
                    skip_count += 1
                    continue
                ...
                if "entity" in existing and conflict_strategy != "overwrite":
                    if conflict_strategy == "skip":
                        ...
                        # Merge observations
                        existing_observations = existing["entity"].get(
                            "observations", []
                        )
                        merged_observations = list(
                            set(existing_observations + observations)
                        )
                ...
                if "error" in result:
                    logger.error(
                        f"Error storing entity {entity_name}: {result['error']}"
                    )
                    error_count += 1
                else:
                    success_count += 1
        ...
        for relationship in relationships:
            try:
                ...
                if not from_entity or not rel_type or not to_entity:
                    logger.warning(
                        f"Skipping relationship with missing data: {relationship}"
                    )
                    continue

                # Create relationship
                result = mcp_client.create_relationship(
                ...
                if "error" in result:
                    ...
                    rel_success_count += 1
                    logger.debug(
                        f"Imported relationship: {from_entity} {rel_type} {to_entity}"
                    )
            except Exception as e:
        ...
        logger.info(
            f"Import summary: {success_count} entities imported, {skip_count} skipped, {error_count} errors"
        )
        logger.info(
            f"Relationship import summary: {rel_success_count} imported, {rel_error_count} errors"
        )
...
def transfer_memory_data(
    source_env: str, target_env: str, conflict_strategy: str = "skip"
) -> bool:
    """
    ...
    try:
        ...
        logger.info(
            f"Successfully transferred memory data from {source_env} to {target_env}"
        )
        return True
    except Exception as e:
...
def parse_args():
    ...
    export_parser.add_argument(
        "--no-relationships", action="store_true", help="Exclude relationships"
    )
    ...
    transfer_parser = subparsers.add_parser(
        "transfer", help="Transfer memory data between environments"
    )
