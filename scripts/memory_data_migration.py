...
def export_memory_data(output_file: str, include_relationships: bool = True) -> bool:
    ...
    try:
        ...
        logger.info(f"Successfully exported {len(entities)} entities and {len(relationships)} relationships to {output_file}")
        return True
...
def parse_args():
    ...
    import_parser.add_argument("--conflict", choices=["skip", "overwrite", "merge"], default="skip",
                             help="Conflict resolution strategy")
    
    # Transfer command
    transfer_parser = subparsers.add_parser("transfer", help="Transfer memory data between environments")
    transfer_parser.add_argument("--source", required=True, choices=["development", "test", "production"],
                               help="Source environment")
    transfer_parser.add_argument("--target", required=True, choices=["development", "test", "production"],
                               help="Target environment")
    transfer_parser.add_argument("--conflict", choices=["skip", "overwrite", "merge"], default="skip",
                               help="Conflict resolution strategy")
