    ...
    for role in roles:
        try:
            ...
            if result.returncode == 0:
                logger.info(f"✅ Successfully added role {role} to {service_account_email}")
            else:
...
def verify_permissions(project_id, location, endpoint_id, service_account_email):
    ...
    try:
        ...
        endpoint_id_short = endpoint_id.split("/")[-1] if "/" in endpoint_id else endpoint_id
        
        command = [
            "gcloud", "ai", "index-endpoints", "describe", endpoint_id_short,
            "--project", project_id,
            "--region", location,
            "--format", "json"
        ]
        ...
        if result.returncode == 0:
            ...
            logger.error(f"❌ Failed to verify permissions.")
            logger.error(f"Error: {result.stderr}")
            return False
    except Exception as e:
...
def main():
    ...
    parser.add_argument("--force", action="store_true", help="Force adding roles even if verification succeeds")
