...
def test_memory_error_handling():
    ...
    try:
        ...
        assert len(messages) == 2, f"Buffer should contain 2 messages, got {len(messages)}"
...
def test_mcp_error_handling():
    ...
    try:
        ...
        with patch.object(mcp, '_trigger_save_workflow', side_effect=Exception("Connection refused")):
            buffer.start_recording()
            ...
            logger.info("Result: %s", result)
            assert "Error" in result, "Should return error message when server is unavailable"
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Test error handling in VANA components')
    parser.add_argument('--verbose', action='store_true', help='Show detailed logs')
    parser.add_argument('--component', choices=['memory', 'vector', 'mcp', 'agent'], 
