...
def main():
    ...
    logger.info("\n# Create the service account key")
    logger.info(
        f"gcloud iam service-accounts keys create secrets/vana-vector-search-sa.json \\\n    --iam-account={service_account_email}"
    )
    ...
    logger.info(
        "%s",
        "sed -i '' 's|GOOGLE_APPLICATION_CREDENTIALS=.*|GOOGLE_APPLICATION_CREDENTIALS=/Users/nick/Development/vana/secrets/vana-vector-search-sa.json|' .env",
    )
