...
def parse_args():
    ...
    parser.add_argument(
        "--env-file", help="Path to .env file to update", default=".env"
    )
    parser.add_argument(
        "--skip-env-update", help="Skip updating the .env file", action="store_true"
    )
...
def get_api_credentials(args):
    """Get API credentials."""
    vector_search_api_url = args.vector_search_api_url or get_input(
        "Vector Search API URL", "https://api.vector-search.example.com"
    )
    vector_search_api_key = args.vector_search_api_key or get_input(
        "Vector Search API key", password=True
    )
    mcp_server_url = args.mcp_server_url or get_input(
...
def get_webhook_credentials(args):
    ...
    webhook_username = args.webhook_username or get_input(
        "Webhook username", "vana_webhook"
    )
    webhook_password = args.webhook_password or get_input(
        "Webhook password", password=True
    )
...
def create_credentials(
    ...
    login_data = {"email": n8n_username, "password": n8n_password}
    response = session.post(login_url, json=login_data)

    if response.status_code != 200:
        logger.error(
            f"Failed to login to n8n: {response.status_code} - {response.text}"
        )
        sys.exit(1)
    ...
    if not vector_search_credential_exists:
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed to create Vector Search API key credential: {response.status_code} - {response.text}"
            )
        else:
    ...
    if not mcp_credential_exists:
        ...
        mcp_credential_data = {
            "name": "MCP API Key",
            "type": "httpHeaderAuth",
            "data": {"name": "Authorization", "value": f"Bearer {mcp_api_key}"},
        }
        response = session.post(credential_url, json=mcp_credential_data)

        if response.status_code != 200:
            logger.error(
                f"Failed to create MCP API key credential: {response.status_code} - {response.text}"
            )
        else:
    ...
    if not webhook_credential_exists:
        ...
        webhook_credential_data = {
            "name": "Webhook Auth",
            "type": "basicAuth",
            "data": {"user": webhook_username, "password": webhook_password},
        }
        response = session.post(credential_url, json=webhook_credential_data)

        if response.status_code != 200:
            logger.error(
                f"Failed to create Webhook Auth credential: {response.status_code} - {response.text}"
            )
        else:
...
def create_environment_variables(
    session, n8n_url, vector_search_api_url, mcp_server_url, mcp_namespace
):
    ...
    if not vector_search_api_url_exists:
        variable_data = {"key": "VECTOR_SEARCH_API_URL", "value": vector_search_api_url}
        response = session.post(variables_url, json=variable_data)

        if response.status_code != 200:
            logger.error(
                f"Failed to create VECTOR_SEARCH_API_URL variable: {response.status_code} - {response.text}"
            )
        else:
            logger.info("Created VECTOR_SEARCH_API_URL variable")

    if not mcp_server_url_exists:
        variable_data = {"key": "MCP_SERVER_URL", "value": mcp_server_url}
        response = session.post(variables_url, json=variable_data)

        if response.status_code != 200:
            logger.error(
                f"Failed to create MCP_SERVER_URL variable: {response.status_code} - {response.text}"
            )
        else:
            logger.info("Created MCP_SERVER_URL variable")

    if not mcp_namespace_exists:
        variable_data = {"key": "MCP_NAMESPACE", "value": mcp_namespace}
        response = session.post(variables_url, json=variable_data)

        if response.status_code != 200:
            logger.error(
                f"Failed to create MCP_NAMESPACE variable: {response.status_code} - {response.text}"
            )
        else:
...
def import_workflows(session, n8n_url, workflows_dir):
    ...
    for workflow_file in workflow_files:
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed to import workflow {workflow_file}: {response.status_code} - {response.text}"
            )
        else:
...
def activate_workflows(session, n8n_url, imported_workflows):
    ...
    for workflow_id, workflow_name in imported_workflows:
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed to activate workflow {workflow_name}: {response.status_code} - {response.text}"
            )
        else:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Failed to get workflow {workflow_name}: {response.status_code} - {response.text}"
                )
            else:
...
def update_env_file(
    env_file, n8n_url, webhook_urls, webhook_username, webhook_password
):
    ...
    for line in env_lines:
        if line.startswith("N8N_WEBHOOK_URL="):
            ...
            line.startswith(f"N8N_WEBHOOK_{workflow.upper().replace(' ', '_')}_URL=")
            for workflow in webhook_urls
        ):
...
def main():
    ...
    if not args.skip_env_update:
        update_env_file(
            args.env_file, n8n_url, webhook_urls, webhook_username, webhook_password
        )
