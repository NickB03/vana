...
def create_credentials(
    n8n_url, n8n_username, n8n_password, vector_search_api_key, mcp_api_key, webhook_username, webhook_password
):
    ...
    if not vector_search_credential_exists:
        vector_search_credential_data = {
            ...
            "type": "httpHeaderAuth",
            "data": {"name": "Authorization", "value": f"Bearer {vector_search_api_key}"},
        }
...
def update_env_file(env_file, n8n_url, webhook_urls, webhook_username, webhook_password):
    ...
    for line in env_lines:
        if line.startswith("N8N_WEBHOOK_URL="):
            ...
            line.startswith(f"N8N_WEBHOOK_{workflow.upper().replace(' ', '_')}_URL=") for workflow in webhook_urls
        ):
