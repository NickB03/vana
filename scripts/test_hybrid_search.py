...
class TestHybridSearch(unittest.TestCase):
    ...
    def _prepare_test_data(self):
        ...
        if self.vector_search_available:
            ...
            self.vector_search_client.upload_embedding(
                vs_only_content, vs_only_metadata
            )
    ...
    def test_vector_search_only(self):
        ...
        with patch.object(self.hybrid_search, "kg_manager") as mock_kg:
            ...
            self.assertTrue(
                len(results.get("vector_search", [])) > 0,
                "No Vector Search results found",
            )
            self.assertEqual(
                len(results.get("knowledge_graph", [])),
                0,
                "Knowledge Graph results found when it should be empty",
            )
            self.assertTrue(
                len(results.get("combined", [])) > 0, "No combined results found"
            )
            ...
            for result in results.get("combined", []):
                self.assertEqual(
                    result.get("source"),
                    "vector_search",
                    "Combined result not from Vector Search",
                )
    ...
    def test_knowledge_graph_only(self):
        ...
        with patch.object(self.hybrid_search, "vector_search_client") as mock_vs:
            ...
            self.assertIsNone(results.get("error"), "Search returned an error")
            self.assertEqual(
                len(results.get("vector_search", [])),
                0,
                "Vector Search results found when it should be empty",
            )
            self.assertTrue(
                len(results.get("knowledge_graph", [])) > 0,
            ...
            for result in results.get("combined", []):
                self.assertEqual(
                    result.get("source"),
                    "knowledge_graph",
                    "Combined result not from Knowledge Graph",
                )
    ...
    def test_combined_results(self):
        ...
        if not self.vector_search_available or not self.kg_available:
            print(
                "Either Vector Search or Knowledge Graph is not available, skipping test"
            )
            return
        ...
        self.assertIsNone(results.get("error"), "Search returned an error")
        self.assertTrue(
            len(results.get("vector_search", [])) > 0, "No Vector Search results found"
        )
        self.assertTrue(
            len(results.get("knowledge_graph", [])) > 0,
            "No Knowledge Graph results found",
        ...
        self.assertTrue(
            "vector_search" in sources, "No Vector Search results in combined results"
        )
        self.assertTrue(
            "knowledge_graph" in sources,
            "No Knowledge Graph results in combined results",
        )

        # Verify results are sorted by score
        scores = [result.get("score", 0) for result in results.get("combined", [])]
        self.assertEqual(
            scores, sorted(scores, reverse=True), "Combined results not sorted by score"
        )
    ...
    def test_vector_search_fallback(self):
        ...
        with patch.object(self.hybrid_search.kg_manager, "query") as mock_query:
            ...
            self.assertEqual(
                len(results.get("knowledge_graph", [])),
                0,
                "Knowledge Graph results found when it should have failed",
            )
            self.assertTrue(
                len(results.get("combined", [])) > 0, "No combined results found"
            )
    ...
    def test_knowledge_graph_fallback(self):
        ...
        with patch.object(
            self.hybrid_search.vector_search_client, "search"
        ) as mock_search:
            ...
            self.assertEqual(
                len(results.get("vector_search", [])),
                0,
                "Vector Search results found when it should have failed",
            )
            self.assertTrue(
                len(results.get("knowledge_graph", [])) > 0,
                "No Knowledge Graph results found",
    ...
    def test_complete_failure(self):
        ...
        with patch.object(
            ...
            with patch.object(self.hybrid_search.kg_manager, "query") as mock_kg_query:
                mock_vs_search.side_effect = Exception(
                    "Simulated Vector Search failure"
                )
                mock_kg_query.side_effect = Exception(
                    "Simulated Knowledge Graph failure"
                )
                ...
                self.assertEqual(
                    len(results.get("combined", [])),
                    0,
                    "Combined results found when both systems failed",
                )
