...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Convert Claude chat history to JSON format"
    )
    parser.add_argument(
        "--input", type=str, required=True, help="Path to Claude chat history text file"
    )
    parser.add_argument(
...
def parse_claude_history(file_path):
    ...
    conversation_pattern = re.compile(
        r"(Conversation: .*?)(?=Conversation: |$)", re.DOTALL
    )
    ...
    if not conversation_matches:
        ...
        logger.info(
            "No conversation markers found. Treating the entire file as one conversation."
        )
    ...
    for i, match in enumerate(conversation_matches):
        ...
        conversation_id = (
            id_match.group(1).strip() if id_match else f"conversation_{i+1}"
        )
        ...
        try:
            timestamp = (
                datetime.strptime(
                    timestamp_match.group(1).strip(), "%Y-%m-%d %H:%M:%S"
                ).isoformat()
                if timestamp_match
        ...
        message_pattern = re.compile(
            r"(Human|Assistant): (.*?)(?=Human:|Assistant:|$)", re.DOTALL
        )
        ...
        if messages:
            conversations.append(
                {"id": conversation_id, "timestamp": timestamp, "messages": messages}
            )
...
def main():
    ...
    logger.info(
        f"You can now import this file using: python import_claude_history.py --input {args.output}"
    )
