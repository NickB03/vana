...
def parse_claude_history(file_path):
    ...
    for i, match in enumerate(conversation_matches):
        ...
        try:
            timestamp = datetime.strptime(timestamp_match.group(1).strip(), '%Y-%m-%d %H:%M:%S').isoformat() if timestamp_match else datetime.now().isoformat()
        except:
    ...
    logger.info(f"Parsed {len(conversations)} conversations with a total of {sum(len(c['messages']) for c in conversations)} messages")
