...
def test_embedding_generation() -> Dict[str, Any]:
    ...
    results = {"success": False, "embedding_dimensions": 0, "error": None}
...
def test_vector_search() -> Dict[str, Any]:
    ...
    results = {"success": False, "results_count": 0, "using_mock": False, "error": None}
...
def generate_permission_fix_instructions(results: Dict[str, Any]) -> str:
    ...
    instructions.append(
        f"gcloud projects add-iam-policy-binding {project_id} --member=serviceAccount:{service_account} --role=roles/aiplatform.serviceAgent"
    )
    instructions.append(
        f"gcloud projects add-iam-policy-binding {project_id} --member=serviceAccount:{service_account} --role=roles/matchingengine.admin"
    )
    instructions.append("```\n")

    instructions.append("## Verify Permissions\n")
    instructions.append(
        "After adding the roles, run this diagnostic tool again to verify the permissions have been applied correctly.\n"
    )
