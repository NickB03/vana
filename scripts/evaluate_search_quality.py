...
def evaluate_search_implementations(test_queries: List[Dict[str, Any]], top_k: int = 5, include_web: bool = False, mock_web: bool = True, verbose: bool = False) -> Dict[str, Any]:
    ...
    for query_data in test_queries:
        ...
        if kg_available:
            ...
            if verbose:
                logger.info(f"Knowledge Graph: {len(formatted_entities)} results, quality: {quality['overall_quality']:.2f}")
        ...
        if verbose:
            logger.info(f"Enhanced Hybrid Search: {len(combined_results)} results, quality: {quality['overall_quality']:.2f}")
        ...
        if verbose:
            logger.info(f"Enhanced Hybrid Search Optimized: {len(combined_results)} results, quality: {quality['overall_quality']:.2f}")
        
        # Add query result to results
        results["query_results"].append(query_result)
    
    # Calculate summary statistics
    for implementation in ["vector_search", "knowledge_graph", "enhanced_hybrid_search", "enhanced_hybrid_search_optimized"]:
...
def generate_report(evaluation_results: Dict[str, Any], output_file: str):
    ...
    with open(output_file, "w") as f:
        ...
        for implementation, data in evaluation_results["summary"].items():
            f.write(f"| {implementation.replace('_', ' ').title()} | {data['average_quality']:.2f} | {data['average_time']:.3f} |\n")
        ...
        for i, query_result in enumerate(evaluation_results["query_results"], 1):
            ...
            for implementation, data in query_result["implementations"].items():
                ...
                f.write(f"| {implementation.replace('_', ' ').title()} | {result_count} | {time_taken:.3f} | {keyword_coverage:.2f} | {overall_quality:.2f} |\n")
