...
def main():
    ...
    rel1_result = kg_manager.store_relationship("VANA", "uses", "Vector Search")
    print(
        f"Stored 'VANA uses Vector Search' relationship: {rel1_result.get('success', False)}"
    )

    # VANA uses Knowledge Graph
    rel2_result = kg_manager.store_relationship("VANA", "uses", "Knowledge Graph")
    print(
        f"Stored 'VANA uses Knowledge Graph' relationship: {rel2_result.get('success', False)}"
    )
    ...
    print("Query for 'VANA uses *':")
    if rel_query.get("relationships"):
        for rel in rel_query.get("relationships", []):
            print(
                f"  - {rel.get('entity1')} {rel.get('relationship')} {rel.get('entity2')}"
            )
    else:
        print("  No results found")
