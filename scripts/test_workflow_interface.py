...
try:
    ...
    try:
        ...
        print("Error: Could not import WorkflowInterface. Make sure the vana package is installed.")
        sys.exit(1)
...
def test_memory_save():
    ...
    logger.info(f"Triggering memory_save workflow with {len(buffer)} messages and tags: {tags}")
...
def test_memory_sync():
    ...
    # Trigger the workflow
    logger.info(f"Triggering memory_sync workflow for user {user_id}, session {session_id}")
...
def test_knowledge_graph_sync():
    ...
    # Trigger the workflow
    logger.info(f"Triggering knowledge_graph_sync workflow with {len(entities)} entities")
