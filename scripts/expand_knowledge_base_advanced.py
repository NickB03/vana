    ...
    try:
        ...
        if add_to_knowledge_graph and kg_manager.is_available():
            try:
                ...
                logger.info(f"  Entities: {kg_result.get('entities_stored', 0)}/{kg_result.get('entities_extracted', 0)}")
                logger.info(f"  Relationships: {kg_result.get('relationships_stored', 0)}/{kg_result.get('relationships_extracted', 0)}")
            except Exception as e:
...
def process_directory_advanced(directory: str, 
