...
def test_update_non_existent_file(setup_auto_indexer):
    ...
    assert indexer.update_indexed_file(file_path, new_content) # The mock will effectively just add it if not found, or overwrite if found.
    assert vector_db.get_document(f"file_{os.path.basename(file_path)}") == new_content # Should now exist after "update" on non-existent
