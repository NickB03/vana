...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test Web Search Functionality for VANA"
    )
    ...
    parser.add_argument(
        "--use-mock", action="store_true", help="Use mock implementation for testing"
    )
    ...
    if args.use_mock:
        try:
            ...
            logger.error(
                "Error importing MockWebSearchClient. Make sure tools/web_search_mock.py exists."
            )
            return 1
    else:
        try:
            ...
            logger.error(
                "Error importing WebSearchClient. Make sure tools/web_search.py exists."
            )
            return 1

    # Check if web search is available
    if (
        hasattr(web_search, "is_available")
        and callable(web_search.is_available)
        and not web_search.is_available()
        and not args.use_mock
    ):
