...
if node tests/hooks/validation/real-prd-validator.js validate .git-hooks-test.tsx .git-hooks-test.tsx > /dev/null 2>&1; then
    ...
    echo "⚠️  PRD validation test had issues (this might be normal for the test file)"
fi
...
# Get final status
node tests/hooks/integration/git-hook-manager.js status 2>/dev/null | grep -A 20 "📊 Git Hook Status:" || echo "Status check completed with warnings"
