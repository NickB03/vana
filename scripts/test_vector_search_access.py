...
def main():
    ...
    if not all([project, location, endpoint_id]):
        ...
        logger.error("Required: GOOGLE_CLOUD_PROJECT, GOOGLE_CLOUD_LOCATION, VECTOR_SEARCH_ENDPOINT_ID")
        return 1
    
    logger.info(f"Testing Vector Search access with:")
    ...
    try:
        ...
        logger.info(f"Successfully accessed index endpoint: {index_endpoint.display_name}")
