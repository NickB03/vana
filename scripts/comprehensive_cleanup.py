...
def create_backup_branch():
    ...
    try:
        ...
        subprocess.run(["git", "checkout", "-b", backup_branch], check=True, capture_output=True)
...
def main():
    ...
    parser.add_argument("--skip-backup", action="store_true", help="Skip creating backup branch")
    parser.add_argument("--skip-git-check", action="store_true", help="Skip git status check")
    parser.add_argument("--phase", choices=["1", "2", "3", "all"], default="all", help="Run specific cleanup phase")
    ...
    if success_count == total_phases:
        ...
        generated_files = [".env.template", "TEST_IMPROVEMENT_PLAN.md"]
