...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("auto_cleanup.log"), logging.StreamHandler()],
)
...
def main():
    ...
    parser.add_argument("--interval", type=int, default=6, help="Hours between cleanup checks (default: 6)")
    parser.add_argument("--threshold", type=int, default=10, help="Minimum duplicates to trigger cleanup (default: 10)")
    parser.add_argument("--daemon", action="store_true", help="Run as daemon (background service)")

    args = parser.parse_args()

    service = AutoCleanupService(interval_hours=args.interval, duplicate_threshold=args.threshold)
