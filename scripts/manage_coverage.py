...
class CoverageManager:
    ...
    def clean_coverage_files(self, keep_days: int = 7, archive: bool = True) -> Dict:
        ...
        for file_path in coverage_files:
            ...
            if file_modified < cutoff_date:
                if archive:
                    # Archive old files
                    archive_name = f"{file_path.name}_{file_modified.strftime('%Y%m%d_%H%M%S')}"
    ...
    def show_history(self, limit: int = 10) -> None:
        ...
        for row in rows:
            ...
            print(f"{timestamp[:19]:<20} {branch[:19]:<20} {commit:<10} {coverage:>6.1f}%")
...
def main():
    ...
    clean_parser.add_argument("--keep-days", type=int, default=7, help="Keep files newer than N days")
    clean_parser.add_argument("--no-archive", action="store_true", help="Delete instead of archive")
    ...
    report_parser.add_argument("--format", choices=["html", "json", "xml"], default="html")
