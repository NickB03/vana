...
def test_vana_orchestration():
    ...
    try:
        ...
        logger.error("Failed to import agent modules. Make sure you're running this script from the project root directory.")
        return False
    ...
    for task in test_tasks:
        ...
        try:
            ...
            if agent == task["expected_agent"]:
                ...
                logger.warning(f"❌ Task incorrectly delegated to {agent}, expected {task['expected_agent']}")
                result = "fail"
...
def test_context_passing():
    ...
    try:
        ...
        logger.error("Failed to import context manager module. Make sure you're running this script from the project root directory.")
        return False
    ...
    try:
        ...
        if deserialized.id == context.id:
            ...
            logger.warning(f"❌ Deserialized context has incorrect ID: {deserialized.id}")
            return False
        
        if deserialized.get_data("test_key") == "test_value":
            logger.info("✅ Deserialized context has correct data")
        else:
            logger.warning(f"❌ Deserialized context has incorrect data: {deserialized.get_data('test_key')}")
            return False
    except Exception as e:
    ...
    try:
        ...
        if loaded.get_data("test_key") == "test_value":
            ...
            logger.warning(f"❌ Loaded context has incorrect data: {loaded.get_data('test_key')}")
            return False
    except Exception as e:
...
def test_result_synthesis():
    ...
    try:
        ...
        logger.error("Failed to import result synthesizer module. Make sure you're running this script from the project root directory.")
        return False
    ...
    try:
        ...
        for i, result in enumerate(ranked):
            logger.info(f"Rank {i+1}: {result['agent']} (confidence: {result['confidence']})")
    except Exception as e:
...
def main():
    ...
    logger.info("\n=== Overall Results ===")
    logger.info(f"Vana Orchestration: {'✅ PASS' if orchestration_result else '❌ FAIL'}")
