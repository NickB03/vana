...
def parse_args():
    ...
    parser.add_argument(
        "--reset", action="store_true", help="Reset memory system (clear local data)"
    )
    parser.add_argument(
        "--seed", action="store_true", help="Seed memory with initial data"
    )
    parser.add_argument(
        "--env",
...
def seed_memory_system(memory_manager):
    ...
    for entity in entities:
        ...
        if result.get("success", False):
            ...
            logger.error(
                f"Failed to store entity {entity['name']}: {result.get('error', 'Unknown error')}"
            )
    ...
    if memory_manager.mcp_available:
        for rel in relationships:
            try:
                result = memory_manager.mcp_client.create_relationship(
                    rel["from"], rel["relationship"], rel["to"]
                )

                if "error" not in result:
                    logger.info(
                        f"Created relationship: {rel['from']} {rel['relationship']} {rel['to']}"
                    )
                else:
                    logger.error(
                        f"Failed to create relationship: {result.get('error', 'Unknown error')}"
                    )
...
def initialize_memory_system(args):
    ...
    if sync_result:
        ...
        logger.warning(
            "Initial synchronization failed or skipped (MCP server not available)"
        )
