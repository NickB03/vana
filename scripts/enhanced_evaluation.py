...
def evaluate_enhanced_hybrid_search(test_queries, top_k=5, include_web=True):
    ...
    try:
        ...
        logger.info(f"Components available: Vector Search: {vs_available}, Knowledge Graph: {kg_available}, Web Search: {web_available}")
    ...
    with open(output_file, "w") as f:
        ...
        for method, results in evaluation_results.items():
            ...
            for result in top_queries:
                f.write(f"| {result['query']} | {result['category']} | {result['difficulty']} | {result['precision']:.2f} | {result['recall']:.2f} | {result['f1']:.2f} | {result['ndcg']:.2f} |\n")
