...
def initialize_local_mcp():
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"❌ Local MCP server is not running or returned status code {response.status_code}")
            print(f"Response: {response.text}")
            print("Make sure the local MCP server is running with 'docker-compose up -d mcp-kg-server'")
            return False
    ...
    entity_types = [
        "person", "project", "document", "concept", "task", 
        "tool", "agent", "knowledge", "memory", "user"
    ]
    
    # Define basic relationship types
    relationship_types = [
        "related_to", "part_of", "created_by", "assigned_to",
        "depends_on", "has_property", "knows_about", "uses"
    ]
