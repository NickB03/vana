...
class MockAgent:
    ...
    def add_tool(self, tool_fn, name, description):
        ...
        self.tools.append(
            {"function": tool_fn, "name": name, "description": description}
        )
    ...
    def run(self, query):
        ...
        if should_search and SEARCH_AVAILABLE:
            ...
            logger.warning(
                "Cannot process knowledge query without search_knowledge_tool"
            )
...
def main():
    ...
    parser.add_argument(
        "--query", default="What is the architecture of VANA?", help="Query to test"
    )
