...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("juno_test_results.log"),
...
def main():
    ...
    parser.add_argument("--output", "-o", type=str, default="juno_test_report.json", 
                        help="Path to output report file")
    parser.add_argument("--single-test", "-s", type=str, 
                        help="Run a single test by ID (e.g., TC001)")
    ...
    if args.single_test:
        # Run a single test
        test_case = next((tc for tc in juno.test_cases if tc["id"] == args.single_test), None)
        if test_case:
