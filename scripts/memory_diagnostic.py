    ...
    try:
        ...
        try:
            result = mcp_client.store_entity(
                test_entity["name"], test_entity["type"], test_entity["observations"]
            )
            ...
            if "success" in result and result["success"]:
                ...
                print(
                    f"❌ Failed to store test entity: {result.get('error', 'Unknown error')}"
                )
        except Exception as e:
        ...
        try:
            ...
            if "entity" in result:
                ...
                print(
                    f"❌ Failed to retrieve test entity: {result.get('error', 'Unknown error')}"
                )
        except Exception as e:
        ...
        try:
            ...
            if "entities" in result:
                print(
                    f"✅ Initial data loaded successfully ({len(result['entities'])} entities)"
                )
            else:
                print(
                    f"❌ Failed to load initial data: {result.get('error', 'Unknown error')}"
                )
        except Exception as e:
        ...
        try:
            ...
            if "added" in result or "modified" in result or "deleted" in result:
                ...
                print(
                    f"✅ Delta sync successful (Added: {added}, Modified: {modified}, Deleted: {deleted})"
                )
            else:
                print(
                    f"❌ Failed to perform delta sync: {result.get('error', 'Unknown error')}"
                )
        except Exception as e:
...
def check_environment_variables():
    ...
    for name, value in variables.items():
        if value:
            print(
                f"✅ {name} is set to: {value[:5]}{'*' * (len(value) - 5) if len(value) > 5 else ''}"
            )
        else:
