...
class VectorBackend(ABC):
    ...
    @abstractmethod
    async def store_vectors(self, documents: List[str], embeddings: List[List[float]], metadata: List[Dict]) -> List[str]:
    ...
    def __init__(self, connection_string: str = None):
        ...
        if not connection_string:
            connection_string = os.getenv('POSTGRES_CONNECTION', 
                'postgresql://user:password@localhost:5432/vana_memory')
