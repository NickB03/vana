...
def test_transfer_to_agent_function():
    ...
    try:
        from vana_multi_agent.tools.standardized_system_tools import (
            standardized_transfer_to_agent,
        )

        # Test 1: Valid agent transfer
        print("\n1. Testing valid agent transfer...")
        result = standardized_transfer_to_agent(
            "architecture_specialist", "Design a new system architecture"
        )
...
def test_adk_tool_integration():
    ...
    try:
        ...
        result = adk_transfer_to_agent.func(
            "devops_specialist", "Deploy the application"
        )
