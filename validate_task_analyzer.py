    ...
    try:
        ...
        for task_text, expected_type in resource_test_cases:
            ...
            if "memory" in analysis.resource_requirements:
                assert isinstance(
                    analysis.resource_requirements["memory"], (int, float, str)
                ), "Memory should be specified appropriately"
            if "cpu" in analysis.resource_requirements:
                assert isinstance(
                    analysis.resource_requirements["cpu"], (int, float, str)
                ), "CPU should be specified appropriately"
        ...
        assert analysis.task_type in [
            ...
            TaskType.GENERAL,
        ], "Should identify reasonable task type"
