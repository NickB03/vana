...
def convert_timezone(datetime_str: str, from_timezone: str, to_timezone: str,
                    input_format: str = "%Y-%m-%d %H:%M:%S") -> str:
    """
...
def calculate_date(base_date: str, operation: str, amount: int, unit: str,
...
def get_time_until(target_datetime: str, target_timezone: str = "UTC",
...
def list_timezones(region: Optional[str] = None) -> str:
    ...
    try:
        ...
        if region:
            ...
            result += "\n\nSupported Abbreviations:\n" + "\n".join([f"{abbr} -> {tz}" for abbr, tz in TIMEZONE_MAPPINGS.items()])
            return result
