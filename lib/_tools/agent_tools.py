...
# Import official ADK AgentTool (Phase 1 - ADK Compliance)
...
# NEW: Create ADK-compliant agent tool (Phase 1 - ADK Compliance)
...
# NEW: Feature-flagged specialist tools creation (Phase 1 - ADK Compliance)
def create_specialist_tools(specialist_agents):
    """
    Create agent tools for a list of specialist agents with feature flag support.
    
    Args:
        specialist_agents: List or dict of specialist agents
        
    Returns:
        List of agent tools (ADK or legacy based on feature flag)
    """
    import os
    
    # Check feature flag
    use_adk_agent_tool = os.getenv("USE_OFFICIAL_AGENT_TOOL", "false").lower() == "true"
    
    if use_adk_agent_tool:
        logger.info("Using official ADK AgentTool implementation")
        # Use new ADK implementation
        ...
        logger.info("Using legacy custom AgentTool implementation")
        # Use existing legacy implementation  
        if isinstance(specialist_agents, dict):
            # Legacy path expects named arguments
            return list(create_specialist_agent_tools(
                specialist_agents.get("architecture"),
                specialist_agents.get("ui"), 
                specialist_agents.get("devops"),
                specialist_agents.get("qa")
            ).values())
        else:
            # Convert list to tools using existing pattern
            tools = []
            for agent in specialist_agents:
                if agent is not None:
                    tool = create_agent_tool(agent)
                    tools.append(tool)
            return tools
