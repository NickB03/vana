"""
Google Custom Search API implementation for ADK (Version 2)
...
def _perform_google_search(query: str, max_results: int) -> str:
    ...
    try:
        ...
        # Add date restriction for time/news queries
        if any(term in query.lower() for term in ["time", "news", "latest", "today"]):
            params["dateRestrict"] = "d1"  # Last day
        else:
...
def google_web_search(query: str, max_results: int) -> str:
    ...
    try:
        ...
        if "time" in query_lower or "clock" in query_lower:
            ...
            if location:
                enhanced_query = f"current time in {location}"
            else:
                enhanced_query = "current time UTC"
        
        # Weather queries
        elif "weather" in query_lower:
            location = _extract_location(query)
            if location:
                enhanced_query = f"weather forecast {location} today"
            else:
                enhanced_query = "weather forecast today"
        
        # Local business/venue queries
        elif any(term in query_lower for term in ["live music", "restaurant", "venue", "near"]):
            if "live music" in query_lower:
                location = _extract_location(query)
                if location:
                    enhanced_query = f"live music venues concerts tonight {location}"
                else:
                    enhanced_query = query + " concerts venues events"
            # Keep other local queries as-is
        
        # Perform the actual search
        result = _perform_google_search(enhanced_query, max_results)
        ...
        try:
            ...
            if enhanced_query != query:
                ...
                data["enhanced_query"] = enhanced_query
            data["cached"] = False
            
            # Special processing for time results
            if "time" in query_lower and data.get("results"):
                for res in data["results"]:
                    snippet = res.get("snippet", "").lower()
                    if "time" in snippet or ":" in snippet:
                        data["extracted_time"] = res["snippet"]
                        break
