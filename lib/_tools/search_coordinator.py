...
class QueryClassifier:
    ...
    def classify_query(query: str) -> Dict[str, Any]:
        ...
        if any(re.search(pattern, query_lower) for pattern in technical_patterns):
            classification.update({"type": "technical", "priority": ["memory", "vector", "web"], "confidence": 0.8})
...
class SearchCoordinator:
    ...
    async def coordinated_search(self, query: str, max_results: int = 5, force_web: bool = False) -> str:
        ...
        try:
            ...
            if force_web or classification.get("requires_current", False):
                ...
                return self._format_coordinated_results(query, [], [], [web_results], classification, start_time)
    ...
    async def _search_memory(self, query: str, max_results: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if memory_data:
                ...
                return [
                    {"source": "memory", "relevance": result.get("score", 0.8), "data": result}
    ...
    async def _search_vector(self, query: str, max_results: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if vector_data:
                ...
                return [
                    {"source": "vector", "relevance": result.get("similarity_score", 0.7), "data": result}
    ...
    def _format_coordinated_results(
        ...
        logger.info(
            f"Coordinated search completed: {len(all_results)} total results from {len(set([r['source'] for r in all_results]))} sources"
        )
        ...
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(asyncio.run, coordinator.coordinated_search(query, max_results))
                    return future.result()
            else:
                return loop.run_until_complete(coordinator.coordinated_search(query, max_results))
        except RuntimeError:
