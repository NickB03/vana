...
class MessageRouter:
    ...
    async def route_task(
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "task": task, "strategy": strategy.value}
    ...
    async def process_queued_tasks(self, agent_name: str) -> Dict[str, Any]:
        ...
        if agent_name not in self.message_queue or not self.message_queue[agent_name]:
            return {"status": "no_tasks", "agent_name": agent_name, "message": "No queued tasks for agent"}
        ...
        for queued_task in queued_tasks:
            try:
                ...
                results.append(
                    {"status": "error", "error": str(e), "task_id": queued_task["task_id"], "task": queued_task["task"]}
                )

        return {"status": "processed", "agent_name": agent_name, "processed_count": len(results), "results": results}
    ...
    async def _select_multiple_agents(
        self, agents: Dict[str, AgentCapability], task: str, count: int, strategy: RoutingStrategy
    ...
    def _update_routing_metrics(
        self, agent_name: str, task: str, strategy: RoutingStrategy, routing_time: float, result: Dict[str, Any]
    ) -> None:
