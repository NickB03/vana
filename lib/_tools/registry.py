...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        self._category_patterns = {
            ToolCategory.ANALYSIS: [
                "analyze", "scan", "evaluate", "assess", "inspect",
                "examine", "review", "check", "validate", "audit"
            ],
            ToolCategory.EXECUTION: [
                "run", "execute", "generate", "create", "write",
                "build", "deploy", "start", "stop", "apply"
            ],
            ToolCategory.INTEGRATION: [
                "api", "search", "fetch", "query", "connect",
                "sync", "pull", "push", "integrate", "webhook"
            ],
            ToolCategory.UTILITY: [
                "format", "transform", "convert", "parse", "encode",
                "decode", "compress", "extract", "filter", "sort"
            ]
        }
    ...
    def get_tools_for_agent(self, agent_type: str, max_tools: int = 6) -> List[FunctionTool]:
        ...
        pattern = agent_patterns.get(agent_type, {
        ...
        for category, count in pattern.items():
            ...
            category_tools.sort(
                key=lambda t: (
                    self.tools[t.name].success_rate,
                    self.tools[t.name].usage_count
                ),
                reverse=True
            )
    ...
    def optimize_tool_assignment(self, agent_type: str, 
                               performance_history: Optional[Dict[str, float]] = None) -> List[FunctionTool]:
        """
