...
class OrchestratorMetrics:
    ...
    def get_summary(self) -> Dict[str, any]:
        ...
        return {
            ...
            "most_used_specialist": max(self.metrics["specialist_calls"].items(), key=lambda x: x[1])[0]
            if self.metrics["specialist_calls"]
            else "none",
            "most_common_task": max(self.metrics["task_types"].items(), key=lambda x: x[1])[0]
            if self.metrics["task_types"]
            else "none",
    ...
    def get_specialist_distribution(self) -> Dict[str, float]:
        ...
        for specialist, count in self.metrics["specialist_calls"].items():
            distribution[specialist] = round((count / self.metrics["total_requests"]) * 100, 1)
        return distribution
...
class MetricsTimer:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            self.metrics.record_error(exc_type.__name__, str(exc_val) if exc_val else "Unknown error")
...
__all__ = ["OrchestratorMetrics", "MetricsTimer", "get_orchestrator_metrics"]
