...
class DynamicAgentFactory:
    ...
    def get_agent_statistics(self) -> Dict[str, Any]:
        ...
        for template_name in self.agent_templates:
            template_agents = [a for a in self.active_agents.values() 
        ...
        if self.active_agents:
            avg_tasks_completed = sum(a.total_tasks_completed for a in self.active_agents.values()) / len(self.active_agents)
            avg_execution_time = sum(a.total_execution_time for a in self.active_agents.values()) / len(self.active_agents)
    ...
    def get_optimization_recommendations(self) -> List[str]:
        ...
        for agent_instance in self.active_agents.values():
            if (agent_instance.state == AgentState.IDLE and 
                current_time - agent_instance.last_used > agent_instance.template.idle_timeout):
