...
class VectorUpdateManager:
    ...
    def update_file_embeddings(self, file_path: Path, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for chunk in new_chunks:
                chunk_id = self.memory.store_chunk(chunk["content"], chunk["metadata"])
                if chunk_id:
    ...
    def cleanup_orphaned_chunks(self) -> Dict[str, int]:
        ...
        results = {"chunks_checked": 0, "orphaned_removed": 0, "files_cleaned": []}
        ...
        try:
            ...
            if orphaned_ids:
                ...
                print(
                    f"🧹 Cleaned {len(orphaned_ids)} orphaned chunks from {len(results['files_cleaned'])} deleted files"
                )
