...
from dashboard.utils.data_formatter import format_percentage, format_currency, format_bytes
...
def display_adk_performance_metrics():
    ...
    with perf_tabs[1]:
        ...
        with col2:
            avg_session_size = metrics['storage']['session_state_size_mb'] / max(metrics['storage']['active_sessions'], 1)
            st.metric(
                "Avg Session Size",
    ...
    with perf_tabs[2]:
        ...
        with col1:
            st.metric(
                "Errors (24h)",
                metrics['reliability']['error_count_24h'],
                delta=None
            )
            st.metric(
                "Success Rate",
    ...
    with perf_tabs[3]:
        ...
        with col2:
            st.metric(
                "Top K Results",
                metrics['configuration']['top_k_results'],
                delta=None
            )
...
def display_adk_cost_analysis():
    ...
    with col1:
        st.metric(
            "Daily Total",
            f"${costs['daily_costs']['total_cost_usd']:.2f}",
            delta=None
        )
    ...
    with col2:
        st.metric(
            "Cost per Query",
            f"${costs['usage']['cost_per_query_usd']:.4f}",
            delta=None
        )
...
def display_adk_session_monitoring():
    ...
    activity_data = pd.DataFrame([
        ...
        {"Metric": "Termination Rate", "Value": session_metrics["session_termination_rate_per_hour"], "Unit": "/hour"}
    ])
