...
class VanaAgentWithMonitoring:
    ...
    def validate_and_process_input(self, user_input: str, source_ip: str = "unknown") -> Dict[str, Any]:
        ...
        try:
            ...
            if not is_valid:
                ...
                return {"success": False, "error": "Input validation failed", "details": validation_message}

            # Rate limiting check
            if not self.security.check_request_rate_limit(source_ip, source_ip):
                self.logger.warning("Rate limit exceeded", agent=self.agent_name, source_ip=source_ip)
            ...
            return {"success": True, "result": result, "processing_time": time.time() - start_time}
            ...
            self.monitoring.record_agent_response(
                self.agent_name, duration, success=True  # You could determine this based on actual success
            )
    ...
    async def async_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info("Starting async task", agent=self.agent_name, task_type=task_data.get("type", "unknown"))
        ...
        self.logger.info("Async task completed", agent=self.agent_name, task_id=result["task_id"])
    ...
    def execute_tool(self, tool_name: str, tool_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            self.logger.debug("Executing tool", agent=self.agent_name, tool=tool_name, params=tool_params)
            ...
            self.logger.error("Tool execution failed", agent=self.agent_name, tool=tool_name, error=str(e))
            result = {"error": str(e)}
            success = False
    ...
    def _mock_tool_execution(self, tool_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        execution_times = {"search_tool": 0.1, "complex_analysis": 0.5, "quick_lookup": 0.05}

        time.sleep(execution_times.get(tool_name, 0.1))

        return {"tool": tool_name, "result": f"Mock result for {tool_name}", "params": params}
