...
# Optional verbose startup diagnostics
DEBUG_STARTUP = os.getenv("VANA_DEBUG") == "1"
...
if DEBUG_STARTUP:
    ...
    if os.path.exists(AGENT_DIR):
        logger.debug(
            f"Agent files in directory: {[f for f in os.listdir(AGENT_DIR) if f.endswith('.py') or f == '__init__.py']}"
        )
        logger.debug(
            f"Directories in agent dir: {[d for d in os.listdir(AGENT_DIR) if os.path.isdir(os.path.join(AGENT_DIR, d)) and not d.startswith('.')]}"
        )
        ...
        try:
            import vana
            logger.debug(f"✅ Root level vana module imported successfully: {vana}")
            logger.debug(f"✅ Root level vana.agent: {vana.agent}")
        except Exception as e:
            logger.debug(f"❌ Failed to import root level vana module: {e}")

        try:
            from agents.vana import agent
            logger.debug(f"✅ agents.vana.agent imported successfully: {agent}")
        except Exception as e:
            logger.debug(f"❌ Failed to import agents.vana.agent: {e}")
