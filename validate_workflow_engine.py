    ...
    try:
        ...
        try:
            from lib._tools.workflow_engine import (
                WorkflowEngine, WorkflowDefinition, WorkflowStep, WorkflowExecution,
                WorkflowState, WorkflowPriority, WorkflowResult, get_workflow_engine
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert all(w["workflow_id"] in [workflow1_id, workflow2_id, workflow3_id] for w in all_workflows), "All workflow IDs should be present"
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            steps = [
                WorkflowStep("step1", "Data Fetch", "Fetch initial data", "fetch_agent", dependencies=[]),
                WorkflowStep("step2", "Process A", "Process data path A", "process_agent", dependencies=["step1"]),
                WorkflowStep("step3", "Process B", "Process data path B", "process_agent", dependencies=["step1"]),
                WorkflowStep("step4", "Merge Results", "Combine A and B", "merge_agent", dependencies=["step2", "step3"]),
                WorkflowStep("step5", "Final Report", "Generate final report", "report_agent", dependencies=["step4"])
            ]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            async def create_test_workflow(index):
                steps = [WorkflowStep(f"step{index}", f"Task {index}", f"Test task {index}", "test_agent")]
            ...
            status_tasks = [asyncio.create_task(asyncio.to_thread(engine.get_workflow_status, wid)) for wid in workflow_ids]
            ...
            assert all(status["state"] == WorkflowState.CREATED.value for status in statuses), "All should be in CREATED state"
