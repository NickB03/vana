...
def test_sequential_execution():
    ...
    try:
        ...
        print(
            f"✅ Session state keys: {list(result.session_state.keys()) if result.session_state else []}"
        )
        ...
        if result.session_state:
            ...
            for key in expected_keys:
                if key in result.session_state:
                    print(
                        f"✅ Session state contains '{key}': {type(result.session_state[key])}"
                    )
                else:
...
def test_sequential_with_real_agents():
    ...
    try:
        ...
        print("✅ Sequential agent created with real agents")
        ...
        result = sequential_agent.execute(
            initial_context="Create a scalable e-commerce platform", session_state={}
        )
        ...
        for agent_name, expected_key in expected_mappings.items():
            if (
                agent_name in result.execution_order
                and expected_key in result.session_state
            ):
...
def main():
    ...
    if passed == total:
        print(
            "🎉 ALL TESTS PASSED! Google ADK Sequential Agent pattern is implemented correctly."
        )
