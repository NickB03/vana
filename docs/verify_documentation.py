...
class DocumentationVerifier:
    ...
    def test_health_endpoint(self):
        ...
        try:
            ...
            if actual == expected:
                ...
                self.results.append(("❌", "Health endpoint", f"Expected {expected}, got {actual}"))
    ...
    def test_port(self):
        ...
        try:
            ...
            try:
                ...
                self.results.append(
                    ("⚠️", "Port 8000", "Server also running on 8000? Documentation may be inconsistent")
                )
            except:
                self.results.append(("✅", "Port 8000", "Correctly not accessible (docs should say 8081)"))
    ...
    def test_api_endpoints(self):
        ...
        # Test /run endpoint
        try:
            response = requests.post(f"{self.base_url}/run", json={"input": "What is 2+2?"}, timeout=10)
            if response.status_code == 200:
                data = response.json()
                if "result" in data and "output" in data["result"]:
                    self.results.append(("✅", "POST /run", "Endpoint working"))
                else:
                    self.results.append(("⚠️", "POST /run", f"Unexpected response format: {data}"))
            else:
                self.results.append(("❌", "POST /run", f"Status {response.status_code}"))
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                self.results.append(("✅", "POST /v1/chat/completions", "OpenAI-compatible endpoint working"))
            else:
                self.results.append(("❌", "POST /v1/chat/completions", f"Status {response.status_code}"))
        except Exception as e:
    ...
    def test_python_version(self):
        ...
        if match:
            ...
            if major == 3 and minor >= 13:
                self.results.append(("✅", "Python version", f"{version} meets requirement (3.13+)"))
            else:
                self.results.append(("❌", "Python version", f"{version} does NOT meet requirement (needs 3.13+)"))
        else:
            self.results.append(("⚠️", "Python version", f"Could not parse version: {version}"))

    def check_tool_availability(self):
        """Verify which tools are actually available"""
        try:
            # Test web search
            response = requests.post(f"{self.base_url}/run", json={"input": "What time is it in London?"}, timeout=15)
            if response.status_code == 200 and "time" in response.text.lower():
                self.results.append(("✅", "Web search tool", "Functional (time query worked)"))
            else:
                self.results.append(("⚠️", "Web search tool", "May not be working properly"))
    ...
    def check_file_references(self):
        ...
        files_to_check = [
            ("docs/validation/GROUND_TRUTH_VALIDATION_REPORT.md", "Referenced in README"),
            ("lib/_tools/web_search_sync.py", "Primary web search implementation"),
        ...
        for filepath, description in files_to_check:
            if os.path.exists(filepath):
                self.results.append(("✅", f"File exists", f"{filepath} - {description}"))
            else:
                self.results.append(("❌", f"Missing file", f"{filepath} - {description}"))
