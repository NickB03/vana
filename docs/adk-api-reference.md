...
interface SSEEvent {
  ...
  content?: {
    parts: Array<{
      ...
      // Function response ⚠️ CRITICAL: Contains agent tool outputs like research plans
      functionResponse?: {
        name: string;
        response: {
          result: string;  // ← Research plans, analysis results live here!
        };
        id: string;
      };
    }>;
...
## ⚠️ CRITICAL: Backend Content Extraction

**IMPORTANT**: When processing ADK events in your backend, you MUST extract content from **BOTH** `text` and `functionResponse` parts. Missing `functionResponse` extraction causes research plans and agent tool outputs to disappear.
...
# Result: Research plans generated by plan_generator are invisible!
```
...
**Why this matters**:
- `plan_generator` and other `AgentTool()` wrapped functions return results via `functionResponse`
- If you only extract `text`, research plans become invisible to users
- This is a recurring bug that breaks core functionality

**See Also**: [ADK Event Extraction Guide](./adk/ADK-Event-Extraction-Guide.md) for complete details and examples.
