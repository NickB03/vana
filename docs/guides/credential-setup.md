...
*   **File Permissions:** Set restrictive file permissions on credential files:
    ```bash
    chmod 600 /path/to/your/credentials.json
    ```
    This ensures only the file owner can read or write to the file.
*   **Encryption at Rest:** Consider encrypting sensitive credential files when not in use, especially on shared systems.

## 6. Dashboard Authentication Credentials

In addition to GCP service account credentials, VANA's dashboard requires authentication credentials when running in production-like mode.

### 6.1 Demo Configuration Credentials

The production-like demo configuration (`dashboard/config/demo.py`) includes default authentication settings:

```python
# Authentication
ENABLE_AUTH = True
DEMO_USERNAME = 'admin'
DEMO_PASSWORD = 'VANA-Demo-2025!'  # Change this for production use
```

For security reasons, you should modify these values before deploying in any persistent environment:

1. **Edit the configuration file:**
   ```bash
   nano dashboard/config/demo.py
   ```

2. **Change the username and password** to secure values.

3. **Update the API key** if API access is needed:
   ```python
   API_KEY_REQUIRED = True
   API_KEY = 'your-secure-api-key-here'
   ```

### 6.2 Generating Secure Credentials

For production use, generate secure random values for passwords and API keys:
...
### 6.3 Environment-Specific Credentials

For different environments (development, testing, production), consider creating separate configuration files:

1. **Create environment-specific configurations:**
   ```bash
   cp dashboard/config/demo.py dashboard/config/production.py
   ```

2. **Update credentials in each file** with environment-appropriate values.

3. **Specify the configuration** when starting the Flask API:
   ```bash
   python dashboard/flask_app.py --config production
   ```

By following these steps, you can securely configure VANA to access the necessary GCP services and protect the dashboard from unauthorized access.
