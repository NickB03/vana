...
class CLIInterface:
    ...
    def setup_validator(self, args) -> ShellValidator:
        ...
        enable_performance = getattr(args, 'performance',
                                    self.config['performance']['enable_performance_mode'])
        ...
        if hasattr(args, 'only_rules') and args.only_rules:
            validator.rules = [rule for rule in validator.rules
                             if rule.rule_id in args.only_rules]
    ...
    def validate_files(self, file_paths: list[str], args) -> list[ValidationResult]:
        ...
        for file_path in file_paths:
            ...
            self.stats['critical_issues'] += len([i for i in result.issues
                                                if i.severity == Severity.CRITICAL])
            self.stats['warning_issues'] += len([i for i in result.issues
                                               if i.severity == Severity.WARNING])
            self.stats['info_issues'] += len([i for i in result.issues
    ...
    def _display_text_results(self, results: list[ValidationResult], args):
        ...
        if self.stats['critical_issues'] > 0:
            print(f"   Critical issues: {ColorFormatter.critical(str(self.stats['critical_issues']))}")
        else:
            print(f"   Critical issues: {self.stats['critical_issues']}")

        if self.stats['warning_issues'] > 0:
            print(f"   Warning issues: {ColorFormatter.warning(str(self.stats['warning_issues']))}")
        else:
            print(f"   Warning issues: {self.stats['warning_issues']}")

        if self.stats['info_issues'] > 0:
            print(f"   Info issues: {ColorFormatter.info(str(self.stats['info_issues']))}")
        else:
    ...
    def _display_grouped_results(self, results: list[ValidationResult], args):
        ...
        # Group by severity
        critical_issues = [(f, i) for f, i in all_issues if i.severity == Severity.CRITICAL]
        warning_issues = [(f, i) for f, i in all_issues if i.severity == Severity.WARNING]
        ...
        try:
            ...
            if args.command == 'validate':
                if args.stdin:
                    ...
                    print(ColorFormatter.critical("❌ No input specified. Use --files, --directory, or --stdin"))
                    return 1
    ...
    def _apply_auto_fixes(self, results: list[ValidationResult], args):
        ...
        for result in results:
            ...
            auto_fixable = [issue for issue in result.issues
            ...
            success, fixes_applied, _ = self.validator.auto_fix_file(result.file_path, backup)
            ...
            if success and fixes_applied > 0:
                ...
                if backup:
                    print(f"   💾 Backup created: {result.file_path}.shell-validator.backup")
            elif not success:
...
Configuration:
  Create .shell-validator.json in your project or home directory for custom settings.
    ...
    validate_parser.add_argument('files', nargs='*', help='Shell script files to validate')
    validate_parser.add_argument('--directory', '-d', help='Directory to scan for shell scripts')
    validate_parser.add_argument('--stdin', action='store_true', help='Read script from stdin')
    validate_parser.add_argument('--format', choices=['text', 'json', 'html'], default='text',
                                help='Output format (default: text)')
    validate_parser.add_argument('--output', '-o', help='Output file (default: stdout)')
    validate_parser.add_argument('--exclude-rules', nargs='+', help='Rule IDs to exclude')
    validate_parser.add_argument('--only-rules', nargs='+', help='Only run specified rule IDs')
    validate_parser.add_argument('--patterns', nargs='+', default=['*.sh', '*.bash', '*.zsh'],
                                help='File patterns to match (default: *.sh *.bash *.zsh)')
    validate_parser.add_argument('--exclude', nargs='+', default=[],
                                help='Patterns to exclude')
    validate_parser.add_argument('--performance', action='store_true', default=True,
                                help='Enable performance mode (default: enabled)')
    validate_parser.add_argument('--auto-fix', action='store_true',
                                help='Automatically fix issues where possible')
    validate_parser.add_argument('--no-backup', action='store_true',
    ...
    hook_parser.add_argument('--hook-type', choices=['pre-commit', 'pre-push'], required=True)
