...
export const ArtifactRenderer = memo(({
  ...
  // Markdown ready effect
  useEffect(() => {
    if (artifact.type === "markdown") {
      onLoadingChange(false);
      window.postMessage({ type: 'artifact-rendered-complete', success: true }, '*');
    }
  }, [artifact.type, onLoadingChange]);

  // Code/HTML iframe message listener
  useEffect(() => {
    if (artifact.type !== "code" && artifact.type !== "html") return;
  ...
  }, [artifact.type, onLoadingChange, onPreviewErrorChange]);

  // Code/HTML timeout fallback
  useEffect(() => {
    if (artifact.type !== "code" && artifact.type !== "html") return;
  ...
  }, [artifact.id, artifact.type, refreshTimestamp, onLoadingChange]);
...
${artifact.content}
    ...
    // Listen for iframe messages - Effect moved to top level
    // Timeout fallback - Effect moved to top level
