...
export class VanaAPIClient {
  ...
  private async performTokenRefresh(): Promise<boolean> {
    try {
      const response = await this.makeRequest<Token>('/auth/refresh', {
        method: 'POST',
  ...
  private async makeRequest<T>(
    ...
    if (!skipAuth && this.refreshToken && this.isTokenExpired()) {
  ...
  async getCurrentUser(): Promise<User> {
    return this.makeRequestWithRetry<User>('/auth/me');
  }
