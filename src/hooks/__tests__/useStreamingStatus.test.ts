...
describe('useStreamingStatus', () => {
  ...
  describe('useStreamingStatusText convenience hook', () => {
    it('returns just the status string', async () => {
      // Dynamic import to avoid top-level circular deps if any (though unlikely here)
      const { useStreamingStatusText } = await import('../useStreamingStatus');
