...
class HookOrchestrator:
        ...
        try:
            ...
            if tool_call.tool_type in [
                ...
                if file_path and (
                    ...
                    error_contexts = (
                        await self.error_capture.capture_typescript_errors()
                    )

                    if error_contexts:
                        self.current_errors.extend(error_contexts)
                        self.validation_metrics["typescript_errors"] += len(
                            error_contexts
                        )

                        # Generate enhanced report
                        enhanced_report = (
                            await self.error_capture.generate_comprehensive_report(
                                self.error_session_id, error_contexts
                            )
                        )
    ...
    async def get_sparc_error_summary(self) -> dict[str, Any]:
        ...
        try:
            return await self.error_capture.get_error_summary_for_sparc(
        ...
        try:
            ...
            if not error_contexts:
                return {
                    ...
                    "sparc_actions": [
                        "Task('Validation Complete', 'All validations passed', 'validator')"
                    ],
                }

            # Generate comprehensive report
            report = await self.error_capture.generate_comprehensive_report(
                f"actionable_{int(time.time())}", error_contexts
            )
            ...
            actionable_report = {
                ...
                "sparc_command_sequence": self.error_capture._generate_sparc_command_sequence(
                    error_contexts
                ),
                "priority_files": self.error_capture._get_priority_files(
                    error_contexts
                ),
