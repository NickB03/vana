    ...
    try:
        ...
        for tool_name in expected_tools:
            if tool_name in specialist_tools:
                ...
                print(
                    f"✅ {tool_name}: {tool.name} with {len(tool.capabilities)} capabilities"
                )
            else:
        ...
        print(
            f"✅ All {len(specialist_tools)} specialist agent tools created successfully"
        )
        return True
...
def test_agent_tool_execution():
    ...
    try:
        ...
        # Create agent tool
        arch_tool = create_agent_tool(
            architecture_specialist, name="test_execution_tool"
        )
    ...
    try:
        ...
        for tool_name, tool in adk_tools.items():
            if hasattr(tool, "func"):
                print(
                    f"✅ ADK {tool_name} tool available with func: {tool.func.__name__}"
                )
            else:
        ...
        for tool_name, tool in adk_tools.items():
            try:
                ...
                print(
                    f"✅ ADK {tool_name} tool executed successfully: {len(result)} chars"
                )
            except Exception as e:
...
def test_vana_orchestrator_tools():
    ...
    try:
        ...
        tool_names = [
            tool.func.__name__ for tool in vana.tools if hasattr(tool, "func")
        ]
...
def main():
    ...
    if passed == total:
        print(
            "🎉 ALL TESTS PASSED! Google ADK Agents-as-Tools pattern is implemented correctly."
        )
