...
def test_free_ai_features():
    ...
    try:
        ...
        if client.is_available():
            print("   ✅ Brave Search client is available")
            ...
            if results:
                ...
                if first_result.get("extra_snippets"):
                    print(
                        f"   ✨ Extra snippets available: {len(first_result['extra_snippets'])}"
                    )
                if first_result.get("ai_summary"):
                    print(
                        f"   🤖 AI summary available: {first_result['ai_summary'][:50]}..."
                    )
                if first_result.get("summary"):
                    print(f"   📝 Summary available: {first_result['summary'][:50]}...")

        else:
            print("   ❌ Brave Search client not available")
    ...
    try:
        ...
        for search_type, query in search_types:
            ...
            try:
                ...
                print(
                    f"   ✅ {search_type.capitalize()} search returned {len(results)} results"
                )

                if results:
                    print(
                        f"   📄 Sample result: {results[0].get('title', 'No title')[:50]}..."
                    )
    ...
    try:
        ...
        for goggle_type, query in goggle_tests:
            ...
            try:
                ...
                print(
                    f"   ✅ {goggle_type.capitalize()} goggle search returned {len(results)} results"
                )
    ...
    try:
        ...
        try:
            ...
            categorized_results = client.multi_type_search(
                "AI agent development", result_types=result_types
            )

            print("   ✅ Multi-type search completed")
...
def test_search_optimization_impact():
    ...
    try:
        ...
        try:
            ...
            optimized_results = client.optimized_search(
                query, search_type="comprehensive"
            )
