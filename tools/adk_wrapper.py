    ...
    def _initialize(self):
        ...
        try:
            ...
            if self.verbose:
                ...
                logger.info(
                    f"ADK module version: {getattr(google.adk, '__version__', 'unknown')}"
                )
            return
        ...
        try:
            ...
            if hasattr(aiplatform, "adk"):
                ...
                if self.verbose:
                    ...
                    logger.info(
                        f"ADK module version: {getattr(aiplatform.adk, '__version__', 'unknown')}"
                    )
                return
            else:
                logger.warning(
        ...
        try:
            ...
            if self.verbose:
                ...
                logger.info(
                    f"Available agent functions: {[f for f in dir(agents) if not f.startswith('_')]}"
                )
            return
        except ImportError as e:
            logger.warning(
    ...
    def run_agent(self, agent, query, **kwargs):
        ...
        if hasattr(agent, "run"):
            ...
            raise AttributeError(
                "Agent does not have 'run' or 'generate_content' method and is not a fallback proxy"
            )
