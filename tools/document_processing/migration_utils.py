...
class MigrationProgressTracker:
    ...
    def get_progress_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "progress_percentage": (self.processed_documents / self.total_documents)
            * 100
            if self.total_documents > 0
            else 0,
            "success_rate": (self.successful_migrations / self.processed_documents)
            * 100
            if self.processed_documents > 0
            else 0,
            "elapsed_time_seconds": elapsed_time,
            "estimated_remaining_seconds": (elapsed_time / self.processed_documents)
    ...
    async def _migrate_single_document(
        ...
        try:
            ...
            if self.config.preserve_metadata or self.config.preserve_chunks:
                converted_document = (
                    self.format_converter.convert_legacy_document_to_adk(
                        legacy_document
                    )
                )
