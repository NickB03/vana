...
from tools.document_processing.adk_document_processor import ADKDocumentProcessor, ADKDocumentMigrator
...
class MigrationProgressTracker:
    ...
    def update_progress(self, mapping: DocumentMapping):
        ...
        # Update counters
        self.processed_documents = len([m for m in self.document_mappings if m.migration_status in ["completed", "failed"]])
    ...
    def get_progress_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "progress_percentage": (self.processed_documents / self.total_documents) * 100 if self.total_documents > 0 else 0,
            "success_rate": (self.successful_migrations / self.processed_documents) * 100 if self.processed_documents > 0 else 0,
            "elapsed_time_seconds": elapsed_time,
            "estimated_remaining_seconds": (elapsed_time / self.processed_documents) * (self.total_documents - self.processed_documents) if self.processed_documents > 0 else 0
        }
...
class ComprehensiveMigrationManager:
    ...
    async def migrate_documents_from_filesystem(self, 
    ...
    async def _migrate_single_document(self, 
                                      file_path: str, 
                                      progress_tracker: MigrationProgressTracker) -> DocumentMapping:
        """
...
async def quick_migrate_directory(source_directory: str, 
                                 adk_processor: ADKDocumentProcessor,
...
def default_migration_progress_callback(progress_info: Dict[str, Any]):
    ...
    logger.info(f"Migration Progress: {percentage:.1f}% ({processed}/{total}) - Success: {successful}, Failed: {failed}")
