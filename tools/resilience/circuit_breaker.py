...
import functools
import threading
from typing import Callable, Any, Dict, Optional, List, Union, TypeVar, cast
        ...
        with self.lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                ...
                if self.failure_count >= self.failure_threshold:
                    old_state = self.state
                    self.state = CircuitState.OPEN
                    logger.warning(f"Circuit breaker '{self.name}' transitioning from CLOSED to OPEN after {self.failure_count} failures")
                    self._notify_state_change(old_state, self.state)
...
class CircuitBreakerRegistry:
    ...
    def get_or_create(self, name: str, failure_threshold: int = 5,
                    reset_timeout: float = 60.0, half_open_max_calls: int = 1) -> CircuitBreaker:
        """
...
def circuit_breaker(name: str, failure_threshold: int = 5,
                  reset_timeout: float = 60.0, half_open_max_calls: int = 1) -> Callable[[Callable[..., T]], Callable[..., T]]:
    """
    Decorator for protecting a function with a circuit breaker.
