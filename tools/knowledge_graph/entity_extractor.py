...
CUSTOM_PATTERNS = [
    ...
    {"label": "technology", "pattern": [{"LOWER": "agent"}, {"LOWER": "development"}, {"LOWER": "kit"}]},
    {"label": "project", "pattern": [{"LOWER": "vana"}]},
    {"label": "project", "pattern": [{"LOWER": "versatile"}, {"LOWER": "agent"}, {"LOWER": "network"}, {"LOWER": "architecture"}]},
...
class EntityExtractor:
    ...
    def _extract_with_spacy(self, text: str) -> List[Dict[str, Any]]:
        ...
        for ent in doc.ents:
            ...
            if entity_key not in seen_entities:
                ...
                sentence = next((sent.text for sent in doc.sents if ent.start >= sent.start and ent.end <= sent.end), "")
                
                # Create entity
                entity = {
                    "name": ent.text,
                    "type": entity_type,
                    "observation": sentence or text[max(0, ent.start_char - 50):min(len(text), ent.end_char + 50)],
    ...
    def _extract_fallback(self, text: str) -> List[Dict[str, Any]]:
        ...
        for pattern_dict in CUSTOM_PATTERNS:
            pattern_text = " ".join([token["LOWER"] for token in pattern_dict["pattern"]])
