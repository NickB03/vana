...
def validate_capability_matcher():
    ...
    try:
        ...
        with patch('lib._tools.capability_matcher.get_discovery_service') as mock_discovery, \
            ...
            assert all(isinstance(w, (int, float)) for w in matcher.capability_weights.values()), \
        ...
        with patch('lib._tools.capability_matcher.get_discovery_service') as mock_discovery, \
            ...
            test_agents = {
                "active": AgentCapability(
                    name="active_agent", description="Active agent",
                    capabilities=["python"], tools=[], status="active",
                    success_rate=0.9
                ),
                "offline": AgentCapability(
                    name="offline_agent", description="Offline agent", 
                    capabilities=["python"], tools=[], status="offline",
                    success_rate=0.0
                )
            }
        ...
        with patch('lib._tools.capability_matcher.get_discovery_service') as mock_discovery, \
            ...
            matches = [
                ...
                CapabilityMatch(
                    ...
                    missing_capabilities=["python", "data_analysis"], capability_coverage=0.33,
                    performance_score=0.8, availability_score=0.9, overall_score=0.65,
