...
def test_local_web_search():
    ...
    try:
        ...
        try:
            ...
            print(
                f"   - Has infobox: {'infobox' in parsed_result and bool(parsed_result['infobox'])}"
            )
            print(
                f"   - Has FAQ: {'faq' in parsed_result and bool(parsed_result['faq'])}"
            )
            print(
                f"   - Has summarizer: {'summarizer' in parsed_result and bool(parsed_result['summarizer'])}"
            )

            # Check individual result fields
            if parsed_result.get("results"):
                first_result = parsed_result["results"][0]
                print("📄 First result enhanced fields:")
                print(
                    f"   - Has extra_snippets: {bool(first_result.get('extra_snippets'))}"
                )
                print(f"   - Has summary: {bool(first_result.get('summary'))}")
                print(f"   - Has age: {bool(first_result.get('age'))}")
                print(
                    f"   - Has relevance_score: {bool(first_result.get('relevance_score'))}"
                )
        ...
        try:
            ...
            if session_response.status_code == 200:
                ...
                message_response = requests.post(
                    run_url, json=message_payload, timeout=45
                )
                ...
                if message_response.status_code == 200:
                    ...
                    if isinstance(response_data, list) and len(response_data) > 0:
                        for event in response_data:
                            if event.get("content") and event.get("content", {}).get(
                                "parts"
                            ):
                                ...
                                for part in parts:
                                    if part.get("text"):
                                        print(
                                            f"   🤖 Agent response: {part['text'][:200]}..."
                                        )
                                        break
                ...
                print(
                    f"   ❌ Session creation failed: HTTP {session_response.status_code}"
                )
