...
class TestVanaIntelligence:
    ...
    async def test_factual_query_reasoning(self, intelligence_validator):
        ...
        result = await intelligence_validator.validate_reasoning_consistency(QueryType.FACTUAL)
    ...
    async def test_tool_selection_intelligence(self, intelligence_validator):
        ...
        result = await intelligence_validator.validate_tool_selection_intelligence(query_types)

        # Verify test execution
        assert result.scenarios_tested > 0, "No scenarios were tested"
        assert result.score >= 0.75, f"Tool selection intelligence too low: {result.score:.2f}"
        ...
        if "factual" in tools_by_type:
            ...
            assert "adk_web_search" in factual_tools or len(factual_tools) == 0, "Factual queries should use web search"
    ...
    @pytest.mark.agent
    @pytest.mark.asyncio
    async def test_response_quality_analysis(self, agent_client, quality_analyzer, test_data_manager):
        """Test VANA's response quality across different scenarios"""
        # Load test scenarios
        try:
            factual_scenarios = test_data_manager.load_scenarios_by_type(QueryType.FACTUAL)
        ...
        for analysis in response_analyses:
            ...
            if analysis["response"]:
                assert len(analysis["response"]) > 10, f"Response too short for query: {analysis['query']}"
    ...
    async def test_context_utilization(self, intelligence_validator):
        ...
        assert good_usage_count >= 0, "Should have some scenarios with good context usage"
    ...
    async def test_error_handling_behavior(self, agent_client):
        ...
        for error_response in error_responses:
            ...
            has_content = bool(error_response["response"] and error_response["response"].strip())
            has_error = bool(error_response["error"])

            assert has_content or has_error, f"No response or error for query: {error_response['query']}"
    ...
    async def test_response_consistency(self, agent_client):
        ...
        if response_lengths:
            ...
            for length in response_lengths:
                ...
                assert 0.3 <= ratio <= 3.0, f"Response length too inconsistent: {length} vs avg {avg_length:.1f}"
    ...
    async def test_performance_characteristics(self, agent_client):
        ...
        assert max_response_time < 30.0, f"Maximum response time too slow: {max_response_time:.2f}s"
    ...
    async def test_intelligence_regression(self, intelligence_validator):
        ...
        tool_result = await intelligence_validator.validate_tool_selection_intelligence()
        ...
        assert overall_intelligence >= 0.7, f"Overall intelligence score too low: {overall_intelligence:.2f}"

        # Individual components should meet minimum thresholds
        assert reasoning_result.score >= 0.6, f"Reasoning consistency below threshold: {reasoning_result.score:.2f}"
        assert tool_result.score >= 0.7, f"Tool selection intelligence below threshold: {tool_result.score:.2f}"
        assert context_result.score >= 0.6, f"Context utilization below threshold: {context_result.score:.2f}"
    ...
    async def test_delegation_intelligence(self, agent_client):
        ...
        for result in delegation_results:
            ...
            if "weather" in query.lower():
                assert "adk_web_search" in tools_used or len(tools_used) == 0, "Weather queries should use web search"
    ...
    async def test_proactive_tool_usage(self, agent_client):
        ...
        for scenario in proactive_scenarios:
            ...
            if response.status == "success" and response.content:
                assert len(response.content) > 50, f"Response too brief for complex query: {scenario}"
