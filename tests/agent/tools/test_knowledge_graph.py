...
class TestKnowledgeGraphTool(unittest.TestCase):
    ...
    def test_query_success(self):
        ...
        mock_entities = [
            {"name": "VANA", "type": "project", "observation": "VANA is an AI project"},
            {
                "name": "Vector Search",
                "type": "technology",
                "observation": "Vector Search is used by VANA",
            },
        ]
    ...
    def test_store_success(self):
        ...
        # Verify the mock was called correctly
        self.mock_kg_manager.store.assert_called_once_with(
            "VANA", "project", "VANA is an AI project"
        )

    def test_store_error(self):
        """Test storing information in the Knowledge Graph with an error."""
        # Set up mock response
        self.mock_kg_manager.store.return_value = {
            "success": False,
            "error": "Store failed",
        }
    ...
    def test_store_relationship_success(self):
        ...
        # Verify the mock was called correctly
        self.mock_kg_manager.store_relationship.assert_called_once_with(
    ...
    def test_extract_entities_success(self):
        ...
        # Verify the mock was called correctly
        self.mock_kg_manager.extract_entities.assert_called_once_with(
            "VANA is a project that uses Vector Search"
        )
    ...
    def test_function_wrappers(self):
        ...
        with patch("agent.tools.knowledge_graph.KnowledgeGraphTool") as mock_tool_class:
            ...
            mock_tool.store.return_value = {
                "success": True,
                "entity": "VANA",
                "type": "project",
            }
            mock_tool.store_relationship.return_value = {
                "success": True,
                "entity1": "VANA",
                "relationship": "uses",
                "entity2": "Vector Search",
            }
            ...
            self.assertTrue(result["success"])
            mock_tool.store.assert_called_once_with(
                "VANA", "project", "VANA is an AI project"
            )
            ...
            self.assertTrue(result["success"])
            mock_tool.store_relationship.assert_called_once_with(
                "VANA", "uses", "Vector Search"
            )
            ...
            self.assertEqual(result, mock_entities)
            mock_tool.extract_entities.assert_called_once_with(
                "VANA is a project that uses Vector Search"
            )
