...
import tempfile
import shutil
from agent.tools.file_system import FileSystemTool, read_file, write_file, list_directory, file_exists
...
class TestFileSystemTool(unittest.TestCase):
    ...
    def test_write_file(self):
        ...
        result = self.file_system_tool.write_file(new_file_path, " Appended", append=True)
        self.assertTrue(result["success"])
        ...
        outside_path = os.path.join(tempfile.gettempdir(), "outside.txt")
        result = self.file_system_tool_with_base.write_file(outside_path, "Outside content")
        self.assertFalse(result["success"])
        self.assertIn("Invalid or restricted", result["error"])
    ...
    def test_list_directory(self):
        ...
        file_entry = next((item for item in contents if item["name"] == "test_file.txt"), None)
        dir_entry = next((item for item in contents if item["name"] == "test_subdir"), None)
    ...
    def test_function_wrappers(self):
        ...
        self.assertEqual(len(contents), 3)  # test_file.txt, test_subdir, and wrapper_test.txt
