...
class PerformanceLoadTester:
    ...
    async def concurrent_load_test(
        ...
        print(
            f"🔥 Load testing {endpoint} with {concurrent_requests} concurrent requests..."
        )
    ...
    async def stress_test(
        self, endpoint: str, duration_seconds: int = 30
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_response_time": avg_duration,
            "requests_per_second": requests_per_second,
            "errors": list({r.get("error") for r in failed if "error" in r}),
        }

    def run_sequential_performance_test(
        self, endpoints: list[str], iterations: int = 100
    ) -> dict[str, Any]:
    ...
    def generate_performance_report(self, results: dict[str, Any]):
        ...
        if "sequential" in results:
            ...
            for endpoint, data in seq_results.items():
                ...
                print(
                    f"      - Min/Max: {data['min_response_time'] * 1000:.1f}ms / {data['max_response_time'] * 1000:.1f}ms"
                )
        ...
        avg_success_rate = (
            statistics.mean(concurrent_success_rates) if concurrent_success_rates else 0
        )
