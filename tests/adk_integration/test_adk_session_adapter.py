...
class TestADKSessionAdapter(unittest.TestCase):
    ...
    def test_add_message_to_session(self):
        ...
        result = self.session_adapter.add_message_to_session(
            context_id=context_id, role="user", content="Hello, world!"
        )
        ...
        # Add another message
        result = self.session_adapter.add_message_to_session(
            context_id=context_id, role="assistant", content="Hi there!"
        )
        ...
        self.assertEqual(
            updated_adk_session.state["messages"][1]["content"], "Hi there!"
        )
    ...
    def test_get_vana_context(self):
        ...
        context = self.session_adapter.get_vana_context(
            adk_session_id="test_session", user_id="test_user"
        )
