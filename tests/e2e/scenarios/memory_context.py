...
class MemoryContextScenario(unittest.TestCase):
    ...
    def test_short_term_memory(self):
        ...
        self.agent.process_message("I prefer detailed explanations and examples.", session_id=self.session_id)
        
        # Step 3: Ask about a topic
        self.agent.process_message("Can you tell me about memory systems in AI?", session_id=self.session_id)
        
        # Step 4: Ask a follow-up question
        self.agent.process_message("How does that relate to knowledge graphs?", session_id=self.session_id)
        
        # Step 5: Ask about user preferences
        response = self.agent.process_message("What are my preferences?", session_id=self.session_id)
        ...
        self.assertEqual(memory_items[2]["content"], "I prefer detailed explanations and examples.")
    ...
    def test_memory_bank_integration(self):
        ...
        section = self.agent.memory_bank.extract_section("activeContext.md", "Current Focus")
        ...
        update_result = self.agent.memory_bank.update_section(
            "activeContext.md", 
            "Current Focus", 
            "This is the updated current focus."
        )
        ...
        updated_section = self.agent.memory_bank.extract_section("activeContext.md", "Current Focus")
        
        # Verify that the section was updated
        self.assertTrue(updated_section["success"])
        self.assertEqual(updated_section["content"], "This is the updated current focus.")
    ...
    def test_combined_memory_workflow(self):
        ...
        self.agent.process_message("My name is Test User and I'm working on the VANA project.", session_id=self.session_id)
        ...
        self.agent.process_message("What is the current focus of the project?", session_id=self.session_id)
        ...
        self.agent.process_message("What tasks have been completed?", session_id=self.session_id)
        
        # Step 6: Ask about the user
        self.agent.process_message("Who am I and what am I working on?", session_id=self.session_id)
        ...
        # Verify the content of the first user message
        self.assertEqual(memory_items[0]["role"], "user")
        self.assertEqual(memory_items[0]["content"], "My name is Test User and I'm working on the VANA project.")
        
        # Verify that the memory bank was updated
        current_focus = self.agent.memory_bank.extract_section("activeContext.md", "Current Focus")
        self.assertEqual(current_focus["content"], "Working on memory integration for the VANA project.")
        
        completed_tasks = self.agent.memory_bank.extract_section("progress.md", "Completed Tasks")
