...
class DashboardTest(TestCase):
    ...
    def _run(self):
        ...
        response = self.execute_step(self.session.get, self.dashboard_url, timeout=10)
        self.assert_true(
            response.status_code == 200, f"Dashboard home page returned status code {response.status_code}"
        )
        ...
        self.assert_true(
            response.status_code == 200, f"Task execution page returned status code {response.status_code}"
        )
