...
class TestAgentWorkflow(unittest.TestCase):
    ...
    def _create_agent(self) -> VanaAgent:
        ...
        mock_get_health_status = MagicMock(return_value={
            "status": "healthy",
        ...
        mock_web_search = MagicMock(return_value=[
            {"title": "Mock web result 1", "snippet": "Mock snippet 1", "url": "https://example.com/1"},
            {"title": "Mock web result 2", "snippet": "Mock snippet 2", "url": "https://example.com/2"}
        ])
    ...
    def test_memory_persistence(self):
        ...
        response = self.agent.process_message("What's my name?", session_id=self.session_id)
        ...
        memory_items = self.agent.short_term_memory.get_all()
        self.assertEqual(len(memory_items), 4)  # Two user messages and two agent responses
        self.assertEqual(memory_items[0]["role"], "user")
    ...
    def test_file_system_tools(self):
        ...
        write_result = self.agent._handle_tool_command(f"!write_file {write_file} {write_content}")
        ...
        # Test list_directory
        list_result = self.agent._handle_tool_command(f"!list_directory {self.test_dir}")
        ...
        # Test file_exists
        exists_result = self.agent._handle_tool_command(f"!file_exists {self.test_file}")
        self.assertTrue(exists_result)
        
        not_exists_result = self.agent._handle_tool_command("!file_exists /path/to/nonexistent/file")
    ...
    def test_vector_search_tools(self):
        ...
        knowledge_result = self.agent._handle_tool_command("!search_knowledge test query")
    ...
    def test_knowledge_graph_tools(self):
        ...
        # Test kg_store
        store_result = self.agent._handle_tool_command("!kg_store VANA project 'VANA is an AI project'")
        self.assertTrue(store_result["success"])
        
        # Test kg_relationship
        relationship_result = self.agent._handle_tool_command("!kg_relationship VANA uses 'Vector Search'")
        self.assertTrue(relationship_result["success"])
        
        # Test kg_extract_entities
        extract_result = self.agent._handle_tool_command("!kg_extract_entities 'VANA is an AI project'")
    ...
    def test_end_to_end_workflow(self):
        ...
        self.agent.process_message("Please remember that VANA is an AI project with memory capabilities.", session_id=self.session_id)
        ...
        self.agent.process_message("Can you search for information about AI agents?", session_id=self.session_id)
        
        # Step 6: Write a file
        file_path = os.path.join(self.test_dir, "summary.txt")
        self.agent.process_message(f"Please write a summary of what we've discussed to {file_path}", session_id=self.session_id)
