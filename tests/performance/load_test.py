...
class VanaLoadTester:
    ...
    def _test_health_endpoint(self):
        ...
        avg_response_time = sum(r.get("response_time", 0) for r in test_results if r.get("success", False)) / max(1, sum(1 for r in test_results if r.get("success", False)))
    ...
    def _generate_summary(self):
        ...
        total_requests = sum(test.get("requests", test.get("total_requests", 0)) for test in self.results["tests"].values())
        overall_success_rate = sum(test.get("success_rate", 0) * test.get("requests", test.get("total_requests", 1)) for test in self.results["tests"].values()) / max(1, total_requests)
        avg_response_time = sum(test.get("avg_response_time", 0) for test in self.results["tests"].values()) / len(self.results["tests"])
