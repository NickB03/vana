...
class TestEnhancedHybridSearchOptimized(unittest.TestCase):
    ...
    def setUp(self):
        ...
        logger.info(
            f"Vector Search available: {self.vector_search_client.is_available()}"
        )
        logger.info(f"Knowledge Graph available: {self.kg_manager.is_available()}")
        logger.info(
            f"Web Search available: {hasattr(self.web_search_client, 'available') and self.web_search_client.available}"
        )
    ...
    def test_query_classification(self):
        ...
        test_cases = [
            ...
            {
                "query": "How to integrate VANA with other systems?",
                "expected_category": "integration",
            },
            {
                "query": "How to evaluate VANA performance?",
            ...
            {
                "query": "How to optimize VANA performance?",
        ...
        for test_case in test_cases:
            ...
            self.assertEqual(
                category,
                expected_category,
                f"Query '{query}' classified as {category}, expected {expected_category}",
            )
    ...
    def test_query_preprocessing(self):
        ...
        test_cases = [
            {
                "query": "What is VANA?",
                "expected_keywords": ["what", "vana", "definition"],
            },
            {
                "query": "How to set up VANA?",
                "expected_keywords": ["how", "set", "up", "vana", "guide"],
            },
            {
                "query": "How does Vector Search work?",
                "expected_keywords": [
        ...
        for test_case in test_cases:
            ...
            for keyword in expected_keywords:
                self.assertIn(
                    keyword,
                    preprocessed.split(),
                    f"Keyword '{keyword}' not in preprocessed query '{preprocessed}'",
                )
    ...
    def test_relevance_calculation(self):
        ...
        for test_case in test_cases:
            ...
            logger.info(
                f"Query: '{query}' - Content: '{content[:30]}...' - Relevance: {relevance:.2f}"
            )
    ...
    def test_diversity_enforcement(self):
        ...
        # Verify diversity
        self.assertGreaterEqual(
            len(source_counts), 2, "Not enough diversity in results"
        )

        # Check for three consecutive results from same source
        for i in range(len(diverse_results) - 2):
            sources = [
                diverse_results[i]["source"],
                diverse_results[i + 1]["source"],
                diverse_results[i + 2]["source"],
            ]
            self.assertFalse(
...
def run_tests():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(
        TestEnhancedHybridSearchOptimized
    )
