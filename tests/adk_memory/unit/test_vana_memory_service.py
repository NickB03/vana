...
class TestVanaMemoryService:
    ...
    def test_initialization_with_valid_config(self, memory_service_config):
        ...
        with patch(
            "vana_multi_agent.memory.vana_memory_service.VertexAiRagMemoryService"
        ) as mock_vertex:
            mock_vertex.return_value = MagicMock()

            with patch(
                "vana_multi_agent.memory.vana_memory_service.VanaMemoryService"
            ) as MockService:
    ...
    async def test_add_session_to_memory_success(
        self, mock_vana_memory_service, mock_session
    ):
        """Test successfully adding session to memory."""
        mock_session.add_message("user", "Hello, how are you?")
        mock_session.add_message("assistant", r"I'm doing well, thank you\!")

        result = await mock_vana_memory_service.add_session_to_memory(mock_session)

        assert result["status"] == "success"
        mock_vana_memory_service.add_session_to_memory.assert_called_once_with(
            mock_session
        )
    ...
    async def test_search_memory_success(self, mock_vana_memory_service):
        ...
        results = await mock_vana_memory_service.search_memory(
            "vana", "test_user", query
        )

        assert len(results) == 1
        assert results[0]["relevance_score"] == 0.9
        assert "ADK memory implementation" in results[0]["content"]
        mock_vana_memory_service.search_memory.assert_called_once_with(
            "vana", "test_user", query
        )
