...
class TestVanaAgent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.agent.state_manager = ADKStateManager(
            self.agent.session_adapter, self.agent.context_manager
        )
    ...
    def test_process_message(self):
        ...
        response = self.agent.process_message(
            user_id="test_user", session_id="test_session", message="Hello, world!"
        )
        ...
        self.assertEqual(
            self.agent.current_context.messages[0]["content"], "Hello, world!"
        )
        self.assertEqual(self.agent.current_context.messages[1]["role"], "assistant")
        self.assertEqual(
            self.agent.current_context.messages[1]["content"], "Test response"
        )
    ...
    def test_generate_response(self):
        ...
        # Create context
        self.agent.create_context(user_id="test_user", session_id="test_session")

        # Mock context_manager.fetch_relevant_memory
        self.agent.context_manager.fetch_relevant_memory = MagicMock(
            return_value=[
        ...
        with patch.object(
            self.agent.__class__, "generate_content", create=True
        ) as mock_generate:
    ...
    def test_conversation_context_preservation(self):
        ...
        # Mock generate_response
        self.agent.generate_response = MagicMock(
            side_effect=["First response", "Second response"]
        )

        # Process first message
        first_response = self.agent.process_message(
            user_id="test_user", session_id="test_session", message="First message"
        )
        ...
        second_response = self.agent.process_message(
            user_id="test_user", session_id="test_session", message="Second message"
        )
        ...
        self.assertEqual(self.agent.current_context.messages[0]["role"], "user")
        self.assertEqual(
            self.agent.current_context.messages[0]["content"], "First message"
        )
        self.assertEqual(self.agent.current_context.messages[1]["role"], "assistant")
        self.assertEqual(
            self.agent.current_context.messages[1]["content"], "First response"
        )
        self.assertEqual(self.agent.current_context.messages[2]["role"], "user")
        self.assertEqual(
            self.agent.current_context.messages[2]["content"], "Second message"
        )
        self.assertEqual(self.agent.current_context.messages[3]["role"], "assistant")
        self.assertEqual(
            self.agent.current_context.messages[3]["content"], "Second response"
        )
