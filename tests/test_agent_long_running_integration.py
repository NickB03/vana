...
def test_agent_has_long_running_tools():
    ...
    try:
        ...
        for tool_name in expected_long_running_tools:
            ...
            ), f"Long-running tool {tool_name} not found in agent tools"

        # Verify total tool count includes long-running tools
        assert (
            len(vana.tools) >= 24
        ), f"Expected at least 24 tools, got {len(vana.tools)}"
...
def test_agent_tool_count():
    ...
    try:
        ...
        file_tools = [
            ...
            if any(
                x in name
                for x in ["read_file", "write_file", "list_directory", "file_exists"]
            )
        ]
        assert (
            len(file_tools) >= 4
        ), f"Expected at least 4 file tools, got {len(file_tools)}"
        ...
        search_tools = [
            ...
            for name in tool_names
            if any(
                x in name for x in ["vector_search", "web_search", "search_knowledge"]
            )
        ]
        assert (
            len(search_tools) >= 3
        ), f"Expected at least 3 search tools, got {len(search_tools)}"
        ...
        system_tools = [
            name
            for name in tool_names
            if any(x in name for x in ["echo", "health_status"])
        ]
        assert (
            len(system_tools) >= 2
        ), f"Expected at least 2 system tools, got {len(system_tools)}"
        ...
        assert (
            len(coord_tools) >= 4
        ), f"Expected at least 4 coordination tools, got {len(coord_tools)}"
        ...
        assert (
            len(lr_tools) >= 4
        ), f"Expected at least 4 long-running tools, got {len(lr_tools)}"
        ...
        agent_tools = [
            ...
            if any(
                x in name
                for x in ["architecture_tool", "ui_tool", "devops_tool", "qa_tool"]
            )
        ]
        assert (
            len(agent_tools) >= 4
        ), f"Expected at least 4 agent tools, got {len(agent_tools)}"

        print(f"Total tools: {len(vana.tools)}")
        print(
            f"Tool breakdown: File({len(file_tools)}), Search({len(search_tools)}), KG({len(kg_tools)}), System({len(system_tools)}), Coord({len(coord_tools)}), LongRunning({len(lr_tools)}), Agents({len(agent_tools)})"
        )
