...
from agent.tools.web_search import search as web_search, search_mock
...
class TestAgentToolsExtendedIntegration(unittest.TestCase):
    ...
    def test_echo_tool(self):
        """Test the echo tool."""
        response = self.agent.process_message("!echo Hello, world!", user_id="test_user")
        self.assertEqual(response, "Echo: Hello, world!")
        
    def test_file_system_tools(self):
        """Test the file system tools."""
        # Test read_file
        response = self.agent.process_message(f"!read_file {self.test_file_path}", user_id="test_user")
        self.assertEqual(response, "Test content")
        
        # Test write_file
        new_file_path = os.path.join(self.test_dir, "new_file.txt")
        response = self.agent.process_message(f"!write_file {new_file_path} New content", user_id="test_user")
        ...
        # Test list_directory
        response = self.agent.process_message(f"!list_directory {self.test_dir}", user_id="test_user")
        self.assertIsInstance(response, list)
        self.assertEqual(len(response), 2)  # test_file.txt and new_file.txt
        
        # Test file_exists
        response = self.agent.process_message(f"!file_exists {new_file_path}", user_id="test_user")
        self.assertTrue(response)
        
        response = self.agent.process_message(f"!file_exists {os.path.join(self.test_dir, 'non_existent.txt')}", user_id="test_user")
    ...
    def test_vector_search_tool(self, mock_search):
        ...
        # Test vector_search
        response = self.agent.process_message("!vector_search test query", user_id="test_user")
        self.assertIsInstance(response, list)
    ...
    def test_web_search_tool(self, mock_search):
        ...
        # Test web_search
        response = self.agent.process_message("!web_search test query", user_id="test_user")
        self.assertIsInstance(response, list)
        self.assertEqual(len(response), 2)
    ...
    def test_tool_error_handling(self):
        ...
        response = self.agent.process_message("!read_file /non/existent/file.txt", user_id="test_user")
        self.assertIsInstance(response, dict)
        self.assertFalse(response["success"])
        self.assertIn("not found", response["error"])
        
        # Test with invalid directory
        response = self.agent.process_message("!list_directory /etc/passwd", user_id="test_user")
        self.assertIsInstance(response, dict)
        self.assertFalse(response["success"])
    ...
    def test_multiple_tools_in_session(self):
        ...
        # Use echo tool
        response1 = self.agent.process_message("!echo First message", session_id=session_id)
        self.assertEqual(response1, "Echo: First message")
        
        # Use file_exists tool
        response2 = self.agent.process_message(f"!file_exists {self.test_file_path}", session_id=session_id)
        self.assertTrue(response2)
        
        # Use read_file tool
        response3 = self.agent.process_message(f"!read_file {self.test_file_path}", session_id=session_id)
    ...
    def test_tool_integration_with_task_parser(self):
        ...
        response = self.agent.process_message("Hello, how are you?", user_id="test_user")
