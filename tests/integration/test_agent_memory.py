...
class TestAgentMemoryIntegration(unittest.TestCase):
    ...
    def test_knowledge_graph_integration(self):
        ...
        # Store an entity
        store_result = self.agent._handle_tool_command("!kg_store Vector_Search technology Vector Search is a semantic search technology")
        ...
        relationship_result = self.agent._handle_tool_command("!kg_relationship VANA uses Vector_Search")
        ...
        # Extract entities
        extract_result = self.agent._handle_tool_command("!kg_extract_entities VANA is a project that uses Vector Search")
    ...
    def test_combined_memory_workflow(self):
        ...
        self.agent.memory_bank.update_section(
            "activeContext.md",
            "Current Focus",
            "Explaining VANA to the user."
        )
        
        # 5. User provides new information
        self.agent.short_term_memory.add("user", "VANA also has Vector Search capabilities.")
        
        # 6. Agent stores new information in knowledge graph
        store_result = self.agent._handle_tool_command("!kg_store Vector_Search technology Vector Search is used by VANA")
        ...
        focus_section = self.agent.memory_bank.extract_section("activeContext.md", "Current Focus")
        self.assertEqual(focus_section["content"], "Explaining VANA to the user.")
        
        progress_section = self.agent.memory_bank.extract_section("progress.md", "Completed Tasks")
        self.assertIn("Explained VANA to the user", progress_section["content"])
        self.assertIn("Learned about Vector Search capabilities", progress_section["content"])
