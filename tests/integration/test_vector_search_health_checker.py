...
class TestVectorSearchHealthChecker:
    ...
    def test_successful_health_check(
        self, vector_search_health_checker, mock_vector_search_client
    ):
    ...
    def test_authentication_failure(
        ...
        assert any(
            issue["severity"] in ["error", "critical"] for issue in result["issues"]
        )
    ...
    def test_recommendation_generation(
        ...
        embedding_recs = [
            rec for rec in recommendations if "embedding" in rec["title"].lower()
        ]
    ...
    def test_trend_calculation(
        ...
        assert report["trends"]["success_rate"]["trend"] in [
            "degrading",
            "improving",
            "stable",
        ]
        assert (
            report["trends"]["success_rate"]["current"]
            < report["trends"]["success_rate"]["previous"]
        )
    ...
    def test_save_report_to_file(
        self, vector_search_health_checker, mock_vector_search_client, tmp_path
    ):
