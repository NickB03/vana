...
from lib._tools.search_coordinator import SearchCoordinator, QueryClassifier
...
class TestSearchCoordinator(unittest.TestCase):
    ...
    async def test_force_web_search(self, mock_web_search):
        ...
        result = await self.coordinator.coordinated_search(
            "test query", 
            force_web=True
        )
    ...
    def test_sufficient_results_logic(self):
        ...
        self.assertTrue(
            self.coordinator._has_sufficient_results(results, vana_classification)
        )
        ...
        self.assertTrue(
            self.coordinator._has_sufficient_results(results, user_classification)
        )
        ...
        self.assertTrue(
            self.coordinator._has_sufficient_results(high_quality_results, general_classification)
        )
        ...
        self.assertFalse(
            self.coordinator._has_sufficient_results(low_quality_results, general_classification)
        )
    ...
    async def test_memory_search_mock(self):
        ...
        mock_memory.search_memory.return_value = [
            {"content": "VANA is an AI system", "score": 0.9}
        ]
    ...
    async def test_vector_search_mock(self):
        ...
        mock_vector.semantic_search_simple.return_value = [
            {"content": "technical content", "similarity_score": 0.8}
        ]
...
if __name__ == "__main__":
    ...
    try:
        result = run_async_test(
            lambda: coordinator.coordinated_search("What is VANA?", max_results=3)
        )
    ...
    try:
        with patch('lib._tools.search_coordinator.web_search') as mock_web:
            ...
            result = run_async_test(
                lambda: coordinator.coordinated_search("test", force_web=True)
            )
