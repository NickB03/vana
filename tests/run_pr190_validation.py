...
def run_security_validation():
    ...
    validation_results = {"timestamp": datetime.utcnow().isoformat(), "tests": []}
    ...
    try:
        ...
        phoenix_auth_checks = [
            ...
            "production" in server_content.lower()
            and "disabled" in server_content.lower(),
            "security_event" in server_content
            or "unauthorized access" in server_content.lower(),
        ]
        ...
        sensitive_data_checks = [
            ...
            "generic error message" in server_content.lower()
            or "service configuration error" in server_content,
            "Security fix:" in server_content,
            "silent=False" not in server_content
            or server_content.count("silent=False") <= 2,  # Limited exposure
        ]
        ...
        cors_checks = [
            ...
            "Security fix: Proper CORS" in server_content
            or "CORS configuration" in server_content,
        ]
        ...
        validation_results["tests"].append(
            {"name": "Server.py Analysis", "error": str(e), "passed": False}
        )
    ...
    try:
        ...
        validation_results["tests"].append(
            {"name": "Auth Guard Analysis", "error": str(e), "passed": False}
        )
    ...
    try:
        ...
        security_commits = [
            ...
            "production-critical" in commit_log.lower()
            or "critical" in commit_log.lower(),
        ]
        ...
        validation_results["tests"].append(
            {"name": "Commit History Analysis", "error": str(e), "passed": False}
        )

    # Calculate overall results
    total_tests = len([t for t in validation_results["tests"] if "error" not in t])
    passed_tests = len(
        [t for t in validation_results["tests"] if t.get("passed", False)]
    )
