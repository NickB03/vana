...
class BackendTester:
    ...
    def test_research_agent_access(self) -> TestResult:
        ...
        try:
            ...
            endpoints = ["/research", "/agent/research", "/api/research", "/research-agent"]
            ...
            return TestResult(
                test_name="Research Agent Access",
                status="ERROR",
                message=f"Error testing research agent: {str(e)}",
                duration=duration
            )
    ...
    def test_error_handling(self) -> TestResult:
        ...
        try:
            ...
            try:
                response_malformed = self.session.post(f"{self.base_url}/list-apps", 
                                                     json={"invalid": "data"})
            except:
            ...
            return TestResult(
                ...
                details={
                    ...
                    "malformed_handled": response_malformed.status_code if response_malformed else "N/A"
                }
            )
        except Exception as e:
            duration = time.time() - start_time
    ...
    def test_adk_ui_specific(self) -> TestResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                has_ui_content = any(keyword in response.text.lower() for keyword in 
                                   ["html", "app", "ui", "interface", "dashboard"])
                
                status = "PASS" if is_html or has_ui_content else "SKIP"
                message = "ADK UI serving correctly" if is_html else "Non-HTML response (API server)"
            ...
            return TestResult(
                test_name="ADK UI Specific",
                status="ERROR",
                message=f"ADK UI test failed: {str(e)}",
                duration=duration
            )
