...
class TestThirdPartyToolRegistry:
    ...
    def test_registry_initialization(self):
        ...
        assert isinstance(
            self.registry.adapters[ThirdPartyToolType.GENERIC], GenericThirdPartyAdapter
        )
    ...
    def test_agent_has_third_party_tools(self):
        ...
        try:
            ...
            for tool_name in expected_third_party_tools:
                ...
                ), f"Third-party tool {tool_name} not found in agent tools"

            # Verify total tool count includes third-party tools
            assert (
                len(vana.tools) >= 30
            ), f"Expected at least 30 tools, got {len(vana.tools)}"
