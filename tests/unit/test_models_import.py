...
def test_models_import_no_side_effects():
    ...
    assert stdout_output == "", (
        f"Unexpected stdout output during import: {stdout_output}"
    )
    assert stderr_output == "", (
        f"Unexpected stderr output during import: {stderr_output}"
    )
...
def test_model_instantiation():
    ...
    resp = ResearchResponse(
        session_id="test-session", status="completed", message="Test completed"
    )
