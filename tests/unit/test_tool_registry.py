...
from lib._tools.registry import (
    ToolRegistry, ToolCategory, ToolMetadata, get_tool_registry
)
...
class TestToolRegistry:
    ...
    def test_get_tools_for_category(self):
        ...
        analysis_tools = [
            create_mock_tool(f"analyze_{i}") for i in range(3)
        ]
        execution_tools = [
            create_mock_tool(f"execute_{i}") for i in range(2)
        ]
    ...
    def test_get_tools_for_agent_security_specialist(self):
        ...
        tools = {
            ToolCategory.ANALYSIS: [
                create_mock_tool(f"analyze_{i}") for i in range(4)
            ],
            ToolCategory.EXECUTION: [
                create_mock_tool(f"execute_{i}") for i in range(2)
            ],
            ToolCategory.INTEGRATION: [
                create_mock_tool(f"api_{i}") for i in range(2)
            ],
            ToolCategory.UTILITY: [
                create_mock_tool(f"format_{i}") for i in range(2)
            ]
        }
    ...
    def test_get_registry_stats(self):
        ...
        for i in range(3):
            registry.register_tool(
                create_mock_tool(f"analyze_{i}"),
                ToolCategory.ANALYSIS
            )
        for i in range(2):
            registry.register_tool(
                create_mock_tool(f"execute_{i}"),
                ToolCategory.EXECUTION
            )
    ...
    def test_optimize_tool_assignment(self):
        ...
        tools = []
        categories = [ToolCategory.ANALYSIS, ToolCategory.EXECUTION, 
                     ToolCategory.INTEGRATION, ToolCategory.UTILITY]
...
class TestToolPatterns:
    ...
    def test_analysis_patterns(self):
        ...
        registry = ToolRegistry()
        analysis_names = [
            "analyze_code", "scan_vulnerabilities", "evaluate_design",
            "assess_risk", "inspect_data", "examine_structure",
            "review_changes", "check_quality", "validate_input", "audit_security"
        ]
    ...
    def test_execution_patterns(self):
        ...
        registry = ToolRegistry()
        execution_names = [
            "run_command", "execute_script", "generate_code",
            "create_file", "write_data", "build_project",
            "deploy_app", "start_service", "stop_process", "apply_changes"
        ]
    ...
    def test_integration_patterns(self):
        ...
        registry = ToolRegistry()
        integration_names = [
            "api_call", "search_web", "fetch_data",
            "query_database", "connect_service", "sync_data",
            "pull_updates", "push_changes", "integrate_system", "webhook_handler"
        ]
    ...
    def test_utility_patterns(self):
        ...
        registry = ToolRegistry()
        utility_names = [
            "format_output", "transform_data", "convert_format",
            "parse_json", "encode_base64", "decode_string",
            "compress_file", "extract_archive", "filter_results", "sort_items"
        ]
