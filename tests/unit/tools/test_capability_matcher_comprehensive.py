...
try:
    from lib._tools.capability_matcher import (
        CapabilityMatch, MatchingResult, CapabilityMatcher,
...
class TestCapabilityMatcherComprehensive:
    ...
    def setup_method(self):
        ...
        self.mock_agents = {
            "vana": AgentCapability(
                ...
                capabilities=["orchestration", "coordination", "workflow_automation", "task_routing"],
            ...
            "data_science": AgentCapability(
                ...
                capabilities=["data_analysis", "statistics", "machine_learning", "python", "visualization"],
    ...
    def test_matching_result_dataclass(self):
        ...
        best_match = CapabilityMatch(
            agent_name="best_agent", match_score=0.9, matched_capabilities=["python"],
            missing_capabilities=[], capability_coverage=1.0, performance_score=0.9,
            availability_score=0.95, overall_score=0.92, reasoning="Perfect match"
        )
    ...
    def test_capability_matcher_initialization(self):
        ...
        with patch('lib._tools.capability_matcher.get_discovery_service') as mock_discovery, \
            ...
            assert all(isinstance(weight, (int, float)) for weight in matcher.capability_weights.values()), \
                "All weights must be numeric"
            assert all(0 <= weight <= 2 for weight in matcher.capability_weights.values()), \
    ...
    def test_calculate_performance_score_functionality(self):
        ...
        with patch('lib._tools.capability_matcher.get_discovery_service') as mock_discovery, \
            ...
            offline_agent = AgentCapability(
                name="offline_agent", description="Offline agent",
                capabilities=["test"], tools=[], status="offline",
                load_factor=1.0, performance_score=0.0
            )
        ...
        with patch('lib._tools.capability_matcher.get_discovery_service') as mock_discovery, \
            ...
            for status, expected_min_score in test_cases:
                agent = AgentCapability(
                    name=f"{status}_agent", description=f"{status} agent",
                    capabilities=["test"], tools=[], status=status,
                    load_factor=0.0, performance_score=1.0
                )
    ...
    def test_analyze_coverage_functionality(self):
        ...
        with patch('lib._tools.capability_matcher.get_discovery_service') as mock_discovery, \
            ...
            match1 = CapabilityMatch(
                agent_name="agent1", match_score=0.8, 
                matched_capabilities=["python", "data_analysis"],
                missing_capabilities=["machine_learning"], capability_coverage=0.67,
                performance_score=0.9, availability_score=0.95, overall_score=0.85,
                reasoning="Good match"
            )
            
            match2 = CapabilityMatch(
                agent_name="agent2", match_score=0.6,
                matched_capabilities=["machine_learning", "visualization"],
                missing_capabilities=["python"], capability_coverage=0.67,
                performance_score=0.8, availability_score=0.9, overall_score=0.75,
            ...
            required_fields = ["total_coverage", "best_coverage", "uncovered_capabilities", "coverage_gaps"]
    ...
    def test_performance_cache_functionality(self):
        ...
        with patch('lib._tools.capability_matcher.get_discovery_service') as mock_discovery, \
            ...
            mock_agent = AgentCapability(
                ...
                capabilities=["test"], tools=[], status="active",
                load_factor=0.0, performance_score=1.0
            )
    ...
    def test_complete_capability_matching_workflow(self):
        ...
        with patch('lib._tools.capability_matcher.get_discovery_service') as mock_discovery, \
            ...
            mock_task_analyzer = Mock()
            mock_analysis = Mock()
            mock_analysis.required_capabilities = ["python", "data_analysis", "visualization"]
            mock_task_analyzer.analyze_task.return_value = mock_analysis
            mock_analyzer.return_value = mock_task_analyzer
