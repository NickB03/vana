...
from lib._tools.agent_tools import (
    # Core Classes
    AgentTool, AgentToolResult, _AgentToolsSingleton,
    # Factory Functions
    create_agent_tool, create_specialist_agent_tools,
    # Tool Functions
    get_adk_architecture_tool, get_adk_ui_tool, get_adk_devops_tool, get_adk_qa_tool,
    # Initialization Functions
    initialize_agent_tools, _get_tool_or_initialize
)
...
class TestAgentToolsComprehensive:
    ...
    def test_agent_tool_get_tool_info_functionality(self):
        ...
        required_fields = ["name", "description", "capabilities", "timeout", "agent_type", "tool_type", "adk_pattern"]
    ...
    def test_create_specialist_agent_tools_functionality(self):
        ...
        assert "Architecture" in result["architecture_tool"].description, "Architecture tool description must be descriptive"
            ...
            for j, result2 in enumerate(results):
                if i != j:
                    ...
                    if "architecture" in result1_lower:
                        assert "ui/ux" not in result1_lower or "devops" not in result1_lower, \
