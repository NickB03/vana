...
class AccessibilityValidator:
    ...
    async def run_accessibility_tests(self) -> dict[str, Any]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                ] = await self.test_visual_accessibility(page)

                # Focus management tests
                self.test_results[
                    "focus_management"
                ] = await self.test_focus_management(page)
    ...
    def run_limited_accessibility_tests(self) -> dict[str, Any]:
        ...
        return {
            "screen_reader": {
                ...
                "details": [
                    {
                        "note": "Playwright not available - manual testing required",
                        "passed": True,
                    }
                ],
                "errors": [],
    ...
    async def test_screen_reader_compatibility(self, page) -> dict[str, Any]:
        ...
        try:
            ...
            for heading in headings:
                ...
                heading_levels.append({"level": tag_name, "text": text_content.strip()})
            ...
            skip_links = await page.query_selector_all(
            ...
            test_results["errors"].append(
                f"Screen reader compatibility test error: {e!s}"
            )
    ...
    async def test_keyboard_navigation(self, page) -> dict[str, Any]:
        ...
        try:
            ...
            modal_triggers = await page.query_selector_all(
                '[data-bs-toggle="modal"], .modal-trigger, button:has-text("open"), button:has-text("show")'
            )
            ...
            if len(modal_triggers) > 0:
                try:
                    ...
                    modals_visible = await page.query_selector_all(
            ...
            test_results["errors"].append(f"Keyboard navigation test error: {e!s}")
    ...
    async def test_aria_labels(self, page) -> dict[str, Any]:
        ...
        try:
            ...
            for landmark in landmarks:
                ...
                tag_name = await landmark.evaluate(
                    "element => element.tagName.toLowerCase()"
                )
                landmark_roles.append(
                    {"tag": tag_name, "role": role or f"implicit {tag_name}"}
                )
            ...
            aria_elements = await page.query_selector_all(
            ...
            for element in aria_elements:
                ...
                if attrs:
                    ...
                    aria_attributes.append({"tag": tag_name, "attributes": attrs})
            ...
            test_results["details"].append(
                {
                    ...
                    "passed": buttons_with_names == len(buttons)
                    if len(buttons) > 0
                    else True,
                }
            )
            ...
            test_results["details"].append(
                {
                    ...
                    "passed": links_with_descriptive_text >= len(links) * 0.8
                    if len(links) > 0
                    else True,  # 80% threshold
                }
            )
            ...
            test_results["errors"].append(f"ARIA labels test error: {e!s}")
    ...
    async def test_visual_accessibility(self, page) -> dict[str, Any]:
        ...
        try:
            ...
            await page.set_viewport_size(
                {
                    "width": int(original_viewport["width"] * 1.5),
                    "height": int(original_viewport["height"] * 1.5),
                }
            )
            ...
            test_results["details"].append(
                {
                    ...
                    "passed": clickable_after_zoom
                    >= min(5, len(interactive_elements)) * 0.8,  # 80% threshold
                }
            )
            ...
            test_results["errors"].append(f"Visual accessibility test error: {e!s}")
        ...
        try:
            ...
            if len(modal_triggers) > 0:
                try:
                    ...
                    test_results["details"].append(
                        {"test": "Modal focus trap", "error": str(e), "passed": False}
                    )
            else:
                test_results["details"].append(
                    {
            ...
            for element in focusable_elements:
                ...
                tab_indices.append(
                    int(tabindex) if tabindex and tabindex.isdigit() else 0
                )
            ...
            test_results["details"].append(
                {
                    ...
                    "passed": len(positive_tabindices)
                    == 0,  # No positive tabindices is better
                }
            )
            ...
            test_results["errors"].append(f"Focus management test error: {e!s}")
    ...
    def generate_accessibility_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "overall_accessibility_status": "ACCESSIBLE"
            if failed_tests == 0
...
async def main():
    ...
    print(
        f"📊 Overall Status: {summary.get('overall_accessibility_status', 'UNKNOWN')}"
    )
...
if __name__ == "__main__":
    ...
    # Save results
    with open(
        "/Users/nick/Development/vana/tests/accessibility_validation_results.json", "w"
    ) as f:
