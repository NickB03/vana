...
class VANAPerformanceBenchmarks:
    ...
    async def run_baseline_tests(self) -> Dict[str, BenchmarkResult]:
        ...
        for test_name, scenario in self.test_scenarios.items():
            ...
            print(f"    ✅ Avg: {result.average_response_time:.2f}s, "
    ...
    async def run_concurrent_user_test(self, test_name: str, scenario: Dict, concurrent_users: int, iterations_per_user: int) -> BenchmarkResult:
        ...
        if all_response_times:
            ...
            p95_response_time = statistics.quantiles(all_response_times, n=20)[18] if len(all_response_times) >= 20 else max_response_time
            p99_response_time = statistics.quantiles(all_response_times, n=100)[98] if len(all_response_times) >= 100 else max_response_time
        else:
            avg_response_time = min_response_time = max_response_time = p95_response_time = p99_response_time = 0.0
    ...
    def analyze_performance_results(self, baseline_results: Dict, load_test_results: Dict) -> Dict[str, Any]:
        ...
        for test_name, result in baseline_results.items():
            analysis["baseline_analysis"][test_name] = {
                "meets_target": result.average_response_time <= self.test_scenarios[test_name]["expected_response_time"],
                "performance_rating": self.rate_performance(result.average_response_time, self.test_scenarios[test_name]["expected_response_time"]),
    ...
    def calculate_performance_degradation(self, baseline: BenchmarkResult, load_test: BenchmarkResult) -> float:
        ...
        degradation = (load_test.average_response_time - baseline.average_response_time) / baseline.average_response_time
        ...
        for test_name, user_results in load_test_results.items():
            ...
            for user_count, result in user_results.items():
                ...
                if degradation > 1.0:  # 100% degradation
                    bottlenecks.append(f"{test_name}: Severe performance degradation ({degradation:.1%}) with {user_count}")
                elif degradation > 0.5:  # 50% degradation
                    bottlenecks.append(f"{test_name}: Significant performance degradation ({degradation:.1%}) with {user_count}")
