...
class TestSecurityVulnerabilityDetection:
    ...
    def teardown_method(self):
        ...
        assert execution_time < 2000, f"Security test took {execution_time:.2f}ms, should be <2000ms"

    @pytest.mark.asyncio
    async def test_xss_vulnerability_detection(self, error_test_workspace, error_patterns):
        ...
        xss_file.write_text(error_patterns['security_vulnerabilities']['xss_vulnerability'])

        content_file = error_test_workspace / 'temp_xss.tsx'
        content_file.write_text(error_patterns['security_vulnerabilities']['xss_vulnerability'])
        ...
        result = subprocess.run([
            ...
            str(xss_file),
            str(content_file)
        ], capture_output=True, text=True, cwd=error_test_workspace)
        ...
        assert validation_result['compliance_score'] < 30  # Very low score for security issue
    ...
    async def test_sql_injection_detection(self, error_test_workspace, error_patterns):
        ...
        content_file.write_text(error_patterns['security_vulnerabilities']['sql_injection'])
        ...
        if result.returncode == 0:
            ...
            logger.info("SQL injection file processed (may need specialized Python validator)")
        else:
            logger.info("SQL injection file validation expected behavior for Python file")
    ...
    async def test_eval_usage_detection(self, error_test_workspace, error_patterns):
        ...
        content_file.write_text(error_patterns['security_vulnerabilities']['eval_usage'])
    ...
    @pytest.mark.asyncio
    async def test_hardcoded_secrets_detection(self, error_test_workspace, error_patterns):
        ...
        secrets_file.write_text(error_patterns['security_vulnerabilities']['hardcoded_secrets'])

        content_file = error_test_workspace / 'temp_secrets.py'
        content_file.write_text(error_patterns['security_vulnerabilities']['hardcoded_secrets'])
        ...
        logger.info("Hardcoded secrets file processed (specialized validator needed for Python)")
...
class TestPerformanceAntiPatternDetection:
    ...
    def teardown_method(self):
        """Verify performance requirements"""
        execution_time = (time.time() - self.start_time) * 1000
        assert execution_time < 1000, f"Performance test took {execution_time:.2f}ms, should be <1000ms"

    @pytest.mark.asyncio
    async def test_excessive_hooks_detection(self, error_test_workspace, error_patterns):
        ...
        hooks_file.write_text(error_patterns['performance_antipatterns']['excessive_hooks'])

        content_file = error_test_workspace / 'temp_hooks.tsx'
        content_file.write_text(error_patterns['performance_antipatterns']['excessive_hooks'])
        ...
        warnings_violations = validation_result.get('warnings', []) + validation_result.get('violations', [])
        warnings_text = ' '.join(warnings_violations)

        # Should detect hook count violations
        assert 'Too many useState' in warnings_text or 'Too many useEffect' in warnings_text

        # Should have suggestions for improvement
        suggestions_text = ' '.join(validation_result.get('suggestions', []))
        assert 'useReducer' in suggestions_text or 'custom hooks' in suggestions_text or 'Combine' in suggestions_text

        logger.info("Excessive hooks correctly detected")

    @pytest.mark.asyncio
    async def test_performance_antipatterns_detection(self, error_test_workspace, error_patterns):
        """Test detection of performance anti-patterns"""
        os.chdir(error_test_workspace)

        # Write component with performance issues
        perf_file = error_test_workspace / 'src/components/PerformanceIssues.tsx'
        perf_file.write_text(error_patterns['performance_antipatterns']['performance_antipatterns'])

        content_file = error_test_workspace / 'temp_perf.tsx'
        content_file.write_text(error_patterns['performance_antipatterns']['performance_antipatterns'])
        ...
        assert performance_suggestions, "Should provide performance improvement suggestions"

        logger.info("Performance anti-patterns correctly detected")

    @pytest.mark.asyncio
    async def test_missing_memoization_detection(self, error_test_workspace, error_patterns):
        ...
        memo_file.write_text(error_patterns['performance_antipatterns']['missing_memoization'])

        content_file = error_test_workspace / 'temp_memo.tsx'
        content_file.write_text(error_patterns['performance_antipatterns']['missing_memoization'])
        ...
        logger.info(f"Memoization analysis completed - suggestions: {len(validation_result.get('suggestions', []))}")
...
class TestAccessibilityViolationDetection:
    ...
    def teardown_method(self):
        """Verify performance requirements"""
        execution_time = (time.time() - self.start_time) * 1000
        assert execution_time < 1000, f"Accessibility test took {execution_time:.2f}ms, should be <1000ms"

    @pytest.mark.asyncio
    async def test_missing_aria_labels_detection(self, error_test_workspace, error_patterns):
        ...
        a11y_file.write_text(error_patterns['accessibility_violations']['missing_aria_labels'])

        content_file = error_test_workspace / 'temp_a11y.tsx'
        content_file.write_text(error_patterns['accessibility_violations']['missing_aria_labels'])
        ...
        assert accessibility_suggestions, "Should provide accessibility improvement suggestions"

        logger.info("Accessibility violations correctly detected")

    @pytest.mark.asyncio
    async def test_keyboard_navigation_issues_detection(self, error_test_workspace, error_patterns):
        ...
        keyboard_file.write_text(error_patterns['accessibility_violations']['keyboard_navigation_issues'])

        content_file = error_test_workspace / 'temp_keyboard.tsx'
        content_file.write_text(error_patterns['accessibility_violations']['keyboard_navigation_issues'])
...
class TestForbiddenUIFrameworkDetection:
    ...
    def teardown_method(self):
        """Verify performance requirements"""
        execution_time = (time.time() - self.start_time) * 1000
        assert execution_time < 1000, f"UI framework test took {execution_time:.2f}ms, should be <1000ms"
    ...
    async def test_material_ui_detection(self, error_test_workspace, error_patterns):
        ...
        mui_file.write_text(error_patterns['forbidden_ui_frameworks']['material_ui_usage'])

        content_file = error_test_workspace / 'temp_mui.tsx'
        content_file.write_text(error_patterns['forbidden_ui_frameworks']['material_ui_usage'])
    ...
    async def test_ant_design_detection(self, error_test_workspace, error_patterns):
        ...
        antd_file.write_text(error_patterns['forbidden_ui_frameworks']['ant_design_usage'])

        content_file = error_test_workspace / 'temp_antd.tsx'
        content_file.write_text(error_patterns['forbidden_ui_frameworks']['ant_design_usage'])
    ...
    async def test_inline_styles_detection(self, error_test_workspace, error_patterns):
        ...
        styles_file.write_text(error_patterns['forbidden_ui_frameworks']['inline_styles_violation'])

        content_file = error_test_workspace / 'temp_styles.tsx'
        content_file.write_text(error_patterns['forbidden_ui_frameworks']['inline_styles_violation'])
...
class TestTypeSafetyIssueDetection:
    ...
    def teardown_method(self):
        """Verify performance requirements"""
        execution_time = (time.time() - self.start_time) * 1000
        assert execution_time < 1000, f"Type safety test took {execution_time:.2f}ms, should be <1000ms"

    @pytest.mark.asyncio
    async def test_missing_typescript_interfaces_detection(self, error_test_workspace, error_patterns):
        """Test detection of missing TypeScript interfaces"""
        os.chdir(error_test_workspace)

        # Write component without TypeScript interfaces
        no_interface_file = error_test_workspace / 'src/components/NoInterfaceComponent.tsx'
        no_interface_file.write_text(error_patterns['type_safety_issues']['missing_typescript_interfaces'])

        content_file = error_test_workspace / 'temp_no_interface.tsx'
        content_file.write_text(error_patterns['type_safety_issues']['missing_typescript_interfaces'])
        ...
        assert type_issues_detected or type_suggestions, "Should detect missing TypeScript interfaces"
...
class TestErrorPatternComprehensiveReport:
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_error_pattern_report(self, error_test_workspace, error_patterns):
        ...
        for category, patterns in error_patterns.items():
            ...
            for pattern_name, pattern_content in patterns.items():
                ...
                if 'import React' in pattern_content or 'export default' in pattern_content:
                    file_ext = '.tsx'
                ...
                # Create test file
                test_file = error_test_workspace / file_dir / f'{pattern_name.title()}{file_ext}'
                ...
                result = subprocess.run([
                    ...
                    str(test_file),
                    str(content_file)
                ], capture_output=True, text=True, cwd=error_test_workspace, timeout=10)
                ...
                if result.returncode == 0:
                    try:
                        ...
                        pattern_result.update({
                            'issues_detected': len(violations) > 0 or len(warnings) > 0,
                        ...
                        if pattern_result['issues_detected'] or pattern_result['compliance_score'] < 80:
        ...
        if detection_report['total_patterns_tested'] > 0:
            detection_report['detection_rate'] = (
                detection_report['patterns_detected'] / detection_report['total_patterns_tested']
            ) * 100
        ...
        logger.info("="*60)
        logger.info(f"Total Patterns Tested: {detection_report['total_patterns_tested']}")
        ...
        for category, results in detection_report['category_breakdown'].items():
            ...
            logger.info(f"{category}: {results['detected']}/{results['total']} ({detection_rate:.1f}%)")
            ...
            for pattern in results['patterns']:
                ...
                logger.info(f"  {pattern['name']}: {status} (Score: {score}, V: {violations}, W: {warnings})")

        logger.info("\n" + "="*60)

        # Save detailed report
        report_file = error_test_workspace / '.claude_workspace' / 'error_detection_report.json'
        report_file.write_text(json.dumps(detection_report, indent=2))

        # Assertions for minimum detection requirements
        assert detection_report['detection_rate'] >= 60, f"Detection rate {detection_report['detection_rate']:.1f}% should be >=60%"

        # Specific category requirements
        security_category = detection_report['category_breakdown'].get('security_vulnerabilities', {})
        if security_category:
            security_rate = (security_category['detected'] / security_category['total']) * 100
            assert security_rate >= 75, f"Security detection rate {security_rate:.1f}% should be >=75%"

        forbidden_ui_category = detection_report['category_breakdown'].get('forbidden_ui_frameworks', {})
        if forbidden_ui_category:
            ui_rate = (forbidden_ui_category['detected'] / forbidden_ui_category['total']) * 100
            assert ui_rate >= 90, f"Forbidden UI detection rate {ui_rate:.1f}% should be >=90%"

        logger.info(f"Comprehensive error pattern testing completed with {detection_report['detection_rate']:.1f}% detection rate")
