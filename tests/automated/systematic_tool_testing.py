...
class VanaToolTester:
    def __init__(self, base_url: str = "https://vana-qqugqgsbcq-uc.a.run.app"):
        ...
        self.create_session_endpoint = (
            f"{base_url}/apps/{self.app_name}/users/{self.user_id}/sessions"
        )
        self.results = {}
    ...
    def create_session(self) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                print(
                    f"❌ Session creation failed: HTTP {response.status_code}: {response.text}"
                )
                return False
    ...
    def test_tool(
        self, tool_name: str, test_message: str, expected_keywords: list[str] = None
    ) -> dict[str, Any]:
    ...
    def run_systematic_tests(self):
        ...
        self.test_tool(
            "echo", "echo systematic testing tool 1", ["systematic", "testing"]
        )
        self.test_tool("health_status", "get health status", ["health", "status"])
        self.test_tool(
            "coordinate_task", "coordinate task for testing", ["coordinate", "task"]
        )
        self.test_tool(
            "ask_for_approval", "ask for approval to proceed", ["approval", "proceed"]
        )
        ...
        self.test_tool(
            "write_file", "write file test.txt with content hello", ["write", "file"]
        )
        self.test_tool(
            "list_directory", "list directory contents", ["list", "directory"]
        )
        self.test_tool(
            "file_exists", "check if file README.md exists", ["file", "exists"]
        )

        # Category 3: Search Tools
        print("\n📁 CATEGORY 3: SEARCH TOOLS (3 tools)")
        self.test_tool(
            "vector_search", "vector search for documentation", ["vector", "search"]
        )
        self.test_tool(
            "web_search", "web search for python tutorials", ["web", "search"]
        )
        self.test_tool(
            "search_knowledge", "search knowledge about AI", ["knowledge", "search"]
        )
        ...
        self.test_tool(
            "generate_report",
            "generate report on system status",
            ["report", "generate"],
        )
        ...
        self.test_tool(
            "architecture_tool",
            "architecture tool design microservices",
            ["architecture", "microservices"],
        )
        self.test_tool("ui_tool", "ui tool design dashboard", ["ui", "dashboard"])
        self.test_tool(
            "devops_tool", "devops tool deployment strategy", ["devops", "deployment"]
        )
    ...
    def print_summary(self):
        ...
        passed_tests = sum(
            1 for result in self.results.values() if result.get("test_passed", False)
        )
