...
def check_existing_rag_corpus(project_id: str, location: str):
    ...
    try:
        ...
        corpus_name = f"projects/{project_id}/locations/{location}/ragCorpora/vana-corpus"
        ...
        try:
            ...
            try:
                ...
                if file_count == 0:
                    logger.warning("⚠️ RAG corpus exists but is empty - no files imported")
...
def import_sample_documents(corpus):
    ...
    try:
        ...
        logger.info("   Note: Import operations are asynchronous and may take a few minutes to complete")
...
def validate_corpus_functionality(corpus):
    ...
    try:
        ...
        try:
            ...
            logger.info("   ⏳ Search functionality validation should be performed after document import completes")
            
        except Exception as search_error:
            logger.warning(f"   ⚠️ Search validation failed (expected if documents still importing): {str(search_error)}")
