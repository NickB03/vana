...
class MCPToolsStatusChecker:
    ...
    async def create_session(self) -> dict[str, Any]:
        ...
        try:
            response = requests.post(
                f"{self.service_url}/sessions", json={"agent_name": "vana"}, timeout=15
            )
    ...
    async def test_mcp_tool(self, tool_name: str, test_message: str) -> dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                ...
                json={"message": f"Use the {tool_name} tool: {test_message}"},
                timeout=30,
            )
            ...
            if response.status_code == 200:
                ...
                tool_used = (
                    tool_name in response_text or "tool" in response_text.lower()
                )
    ...
    async def run_comprehensive_check(self) -> dict[str, Any]:
        ...
        if health_result["status"] != "healthy":
            return {
                "overall_status": "service_unhealthy",
        ...
        for tool_name, test_message in core_mcp_tools:
            ...
            if result["status"] == "success":
                successful_core += 1
                print(
                    f"   ✅ {tool_name} - SUCCESS ({result.get('response_time', 0)}s)"
                )
            elif result["status"] == "tool_not_used":
                print(
                    f"   ⚠️ {tool_name} - RESPONDED BUT TOOL NOT USED ({result.get('response_time', 0)}s)"
                )
            else:
                print(
                    f"   ❌ {tool_name} - FAILED ({result.get('error', result.get('status', 'unknown'))})"
                )
        ...
        print(
            f"Time Tools: {successful_time}/{len(time_tools)} ({successful_time/len(time_tools)*100:.1f}%)"
        )
        print(
            f"Filesystem Tools: {successful_filesystem}/{len(filesystem_tools)} ({successful_filesystem/len(filesystem_tools)*100:.1f}%)"
        )
        print(
            f"OVERALL SUCCESS RATE: {success_rate:.1f}% ({total_successful}/{total_tested})"
        )
