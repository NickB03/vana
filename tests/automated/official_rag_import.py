...
def import_rag_files_from_gcs(
    ...
    try:
        ...
        for path in paths:
            ...
            while attempt < max_retries and imported < num_files_to_be_imported:
                ...
                try:
                    ...
                    logger.info(
                        f"   📊 Imported: {imported}/{num_files_to_be_imported}"
                    )
        ...
        logger.info(
            f"🎉 Import completed: {total_imported} files out of {total_num_of_files} imported!"
        )
...
'''
    ...
    logger.info(
        "📄 Official Cloud Function code saved to cloud_function_official_rag_import.py"
    )
    logger.info("")
...
def main():
    ...
    logger.info(
        "   Based on: https://github.com/GoogleCloudPlatform/generative-ai/tree/main/gemini/rag-engine"
    )
    ...
    try:
        import_rag_files_from_gcs(
            paths=GCS_FILES, chunk_size=512, chunk_overlap=50, corpus_name=CORPUS_NAME
        )
