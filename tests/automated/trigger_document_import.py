...
async def import_documents_with_adk():
    ...
    try:
        ...
        processor = ADKDocumentProcessor(
            project_id=PROJECT_ID, location=LOCATION, rag_corpus_name=CORPUS_NAME
        )
        ...
        for gcs_uri in GCS_FILES:
            ...
            try:
                ...
                result = processor.import_files_from_gcs(
                    gcs_uri=gcs_uri, chunk_size=512, max_embedding_requests_per_min=1000
                )

                if result.get("success"):
                    logger.info(f"   ✅ Successfully imported: {gcs_uri}")
                    logger.info(
                        f"      Operation: {result.get('import_response', {}).get('operation_id', 'unknown')}"
                    )
                else:
...
'''
    ...
    logger.info(
        "   2. Set trigger to Cloud Storage (bucket: analystai-454200-vector-search-docs)"
    )
