...
class ComprehensiveMCPValidator:
    ...
    async def _test_core_mcp_tools(self) -> dict[str, Any]:
        ...
        core_tools = [
            ...
            (
                ...
                "Use Context7 for structured analysis of MCP tools",
            ),
            (
                "brave_search_mcp",
                "Search for 'Model Context Protocol' using Brave Search",
            ),
    ...
    async def _test_single_tool(
        self, tool_name: str, test_query: str
    ) -> MCPToolTestResult:
        ...
        try:
            ...
            payload = {
                ...
                "newMessage": {
                    "parts": [{"text": f"Use the {tool_name} tool: {test_query}"}],
                    "role": "user",
                },
                "streaming": False,
            }
    ...
    def _generate_validation_report(
        ...
        for category, cat_result in category_results.items():
            if cat_result["status"] == "success" and isinstance(
                cat_result.get("results"), dict
            ):
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                success_rate, category_results
            ),
            "next_steps": self._generate_next_steps(success_rate),
        }

    def _generate_recommendations(
        self, success_rate: float, category_results: dict[str, Any]
    ) -> list[str]:
        ...
        if success_rate >= 90:
            recommendations.append(
                "✅ MCP tools implementation is excellent - ready for MVP completion"
            )
            recommendations.append("🚀 Proceed to Phase 4: Frontend GUI implementation")
        elif success_rate >= 75:
            recommendations.append(
                "⚠️ MCP tools mostly functional - address failing tools before MVP"
            )
            recommendations.append("🔧 Focus on fixing specific tool issues identified")
        else:
            recommendations.append(
                "🚨 Significant MCP tools issues - requires immediate attention"
            )
