...
class ComprehensiveMCPValidator:
    ...
    async def _test_single_tool(self, tool_name: str, test_query: str) -> MCPToolTestResult:
        ...
        try:
            ...
            payload = {
                ...
                "newMessage": {"parts": [{"text": f"Use the {tool_name} tool: {test_query}"}], "role": "user"},
                "streaming": False,
            }

            # Make API call to VANA service using /run endpoint
            response = self.session.post(
                f"{self.service_url}/run", json=payload, timeout=30, headers={"Content-Type": "application/json"}
            )
    ...
    def _check_tool_usage(self, response_data: Dict[str, Any], tool_name: str) -> bool:
        ...
        tool_indicators = [tool_name.lower(), "tool", "function", "executed", "called", "used"]
