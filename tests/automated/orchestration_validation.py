...
class OrchestrationValidator:
    ...
    async def test_architecture_delegation(self, page: Page) -> Dict[str, Any]:
        ...
        try:
            ...
            has_architecture_content = any(
                ...
                for keyword in ["microservices", "architecture", "api gateway", "database", "service"]
            )
            ...
            status = (
                "PASS"
                if (
                    (has_architecture_tool or has_architecture_content)
                    and no_transfer
                    and no_user_transfer
    ...
    async def test_ui_delegation(self, page: Page) -> Dict[str, Any]:
        ...
        try:
            ...
            has_ui_content = any(
                keyword in response_text.lower()
                for keyword in ["dashboard", "ui", "interface", "design", "component", "dark mode"]
            )
            no_transfer = "transfer_to_agent" not in response_text.lower()
            no_user_transfer = "transferring" not in response_text.lower()
            ...
            status = (
                "PASS"
                if ((has_ui_tool or has_ui_content) and no_transfer and no_user_transfer and response_time < 10.0)
                else "FAIL"
            )
    ...
    async def test_devops_delegation(self, page: Page) -> Dict[str, Any]:
        ...
        try:
            ...
            has_devops_content = any(
                keyword in response_text.lower()
                for keyword in ["deployment", "docker", "kubernetes", "ci/cd", "pipeline", "infrastructure"]
            )
            no_transfer = "transfer_to_agent" not in response_text.lower()

            status = (
                "PASS" if ((has_devops_tool or has_devops_content) and no_transfer and response_time < 10.0) else "FAIL"
            )
    ...
    async def test_tool_functionality(self, page: Page) -> Dict[str, Any]:
        ...
        try:
            ...
            has_time_info = any(
                keyword in response_text.lower() for keyword in ["time", "clock", "utc", "am", "pm", "2025"]
            )

            status = "PASS" if ((has_time_tool or has_time_info) and response_time < 5.0) else "FAIL"
