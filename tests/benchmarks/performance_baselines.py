...
class PerformanceBaselines:
    ...
    def add_baseline(self, baseline: PerformanceBaseline):
        ...
        logger.info(f"Added baseline for {baseline.benchmark_name}.{baseline.metric_name}: {baseline.baseline_value} {baseline.unit}")
    ...
    def update_baseline(self, benchmark_name: str, metric_name: str, new_baseline: PerformanceBaseline):
        ...
        if key in self.baselines:
            ...
            logger.info(f"Updating baseline for {benchmark_name}.{metric_name}: "
...
class BaselineManager:
    ...
    def establish_baseline(self, benchmark_name: str, metric_name: str, values: List[float], 
                          unit: str, confidence_level: float = 0.95) -> PerformanceBaseline:
    ...
    def compare_to_baseline(self, benchmark_name: str, metric_name: str, 
    ...
    def update_baseline_if_needed(self, benchmark_name: str, metric_name: str, 
                                 new_values: List[float], unit: str,
                                 auto_update_threshold: float = 0.1) -> bool:
        ...
        percentage_change = abs((new_mean - baseline.baseline_value) / baseline.baseline_value) if baseline.baseline_value != 0 else 0
        
        if percentage_change > auto_update_threshold:
            # Significant change detected, update baseline
            logger.info(f"Significant performance change detected for {benchmark_name}.{metric_name}: "
