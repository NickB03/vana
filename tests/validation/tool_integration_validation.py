...
class ToolIntegrationValidator:
    ...
    def __init__(self):
        ...
        self.tool_categories = {
            "base_adk_tools": {
                "tools": ["echo", "read_file", "write_file", "list_directory", "file_exists", 
            ...
            "specialist_tools": {
                "tools": ["competitive_intelligence_tool", "web_research_tool", "code_generation_tool"],
                "critical": True,
                "description": "Specialist agent tools"
            },
            "long_running_tools": {
                "tools": ["ask_for_approval", "process_large_dataset", "check_task_status"],
                "critical": False,
    ...
    async def _test_tool_categories(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for category_name, category_config in self.tool_categories.items():
                ...
                logger.info(f"   📊 {category_name}: {category_test_result['tools_passed']}/{total_tools} tools passed ({category_test_result['success_rate']:.1%})")
    ...
    async def _test_individual_tools(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"   📊 Individual Tools: {successful_tools}/{total_tools} high-priority tools passed ({individual_results['individual_success_rate']:.1%})")
    ...
    async def _collect_tool_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            benchmarks = {
                "file_operations": {"read_file": 0.05, "write_file": 0.08, "list_directory": 0.03},
                "search_operations": {"vector_search": 0.15, "web_search": 0.25, "search_knowledge": 0.12},
                "coordination_operations": {"coordinate_task": 0.20, "delegate_to_agent": 0.18, "get_agent_status": 0.05},
            ...
            for category, tools in benchmarks.items():
                ...
                for tool_name, expected_time in tools.items():
                    ...
                    category_results[tool_name] = {
                        ...
                        "variance_percent": ((actual_time - expected_time) / expected_time) * 100 if expected_time > 0 else 0
                    }
            ...
            if self.baseline_manager.baselines.baselines:
                ...
                if tool_baseline_key in self.baseline_manager.baselines.baselines:
                    ...
                    percentage_diff = ((current_avg_time - baseline_value) / baseline_value) * 100 if baseline_value > 0 else 0

                    baseline_comparison["tool_execution_time"] = {
                        "current": current_avg_time,
                        "baseline": baseline_value,
                        "difference_percent": percentage_diff,
                        "status": "improved" if percentage_diff < 0 else "degraded" if percentage_diff > 10 else "stable"
                    }

                    if percentage_diff > 25:
                        performance_results["performance_issues"].append(f"Tool execution time degraded by {percentage_diff:.1f}%")
            ...
            if max_response_time < self.validation_config["performance_threshold"] and avg_response_time < 0.2:
                ...
                performance_results["performance_issues"].append(f"Some tools exceed {self.validation_config['performance_threshold']}s threshold")

            logger.info(f"   📊 Performance Status: {performance_results['performance_status']} (avg: {avg_response_time:.3f}s)")
        ...
        try:
            ...
            scenarios = [
                ...
                },
                {
                    ...
                    "tools": ["coordinate_task", "delegate_to_agent", "get_agent_status"],
            ...
            integration_results["integration_success_rate"] = successful_scenarios / total_scenarios if total_scenarios > 0 else 0.0
            ...
            logger.info(f"   📊 Integration Scenarios: {successful_scenarios}/{total_scenarios} passed ({integration_results['integration_success_rate']:.1%})")
    ...
    def _generate_tool_validation_summary(self, validation_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "tool_category_results" in validation_results:
                ...
                for category, status in critical_status.items():
                    if not status.get("meets_threshold", False):
                        summary["critical_issues"].append(f"Critical tool category '{category}' below threshold: {status.get('success_rate', 0):.1%}")
            ...
            tool_categories = ["base_adk_tools", "coordination_tools", "specialist_tools"]
