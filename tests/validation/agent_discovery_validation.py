...
class AgentDiscoveryValidator:
    ...
    async def _validate_agent_coordination(self) -> Dict[str, Any]:
        ...
        try:
            ...
            all_coord_tests = (
                coordination_results["delegation_tests"]
            ...
            coordination_results["coordination_success_rate"] = (
                successful_coord / total_coord if total_coord > 0 else 0.0
            )

            response_times = [test["response_time"] for test in all_coord_tests]
            coordination_results["coordination_response_time"] = (
                sum(response_times) / len(response_times) if response_times else 0.0
            )
            ...
            if "agent_discovery" in validation_results:
                ...
                if missing_critical:
                    summary["critical_issues"].append(
                        f"Missing critical agents: {[a['name'] for a in missing_critical]}"
                    )
