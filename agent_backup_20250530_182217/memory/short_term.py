...
class ShortTermMemory:
    ...
    def __init__(self, max_items: int = 100, max_age_seconds: Optional[int] = None):
        ...
        logger.info(
            f"Initialized ShortTermMemory with max_items={max_items}, max_age_seconds={max_age_seconds}"
        )
    ...
    def add(
        ...
        logger.debug(
            f"Added item to short-term memory. Buffer size: {len(self.memory_buffer)}"
        )
        return memory_item
    ...
    def get_recent(
        self, count: int = 5, filter_role: Optional[str] = None
    ) -> list[dict[str, Any]]:
        """
        ...
        for i in range(len(self.memory_buffer) - 1):
            ...
            if current["role"] == "user" and next_item["role"] == "assistant":
                conversation_pairs.append(
                    {"user": current["content"], "assistant": next_item["content"]}
                )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        """
        if not self.memory_buffer:
            return {"count": 0, "oldest": None, "newest": None, "roles": {}}
        ...
        # Get timestamps
        timestamps = [
            datetime.fromisoformat(item["timestamp"]) for item in self.memory_buffer
        ]
